(function() {
    ...
    var stopNoticeShown = false;
    var REDIRECT_GUARD_KEY = 'app_settings_redirect_guard_v1';
    var REDIRECT_LOOP_WINDOW_MS = 120000;
    ...
    function targetHostname(targetUrl) {
        try {
            ...
            return normalizeHostname(parser.hostname || parser.host);
        } catch (e) {
            return '';
        }
    }

    // Протокол і стандартний порт можуть відрізнятися, але сервер лишається тим самим.
    function isAlreadyOnTargetServer(targetUrl) {
        var currentHostname = normalizeHostname(window.location.hostname);
        return currentHostname !== '' && currentHostname === targetHostname(targetUrl);
    }

    function guardStores() {
        var stores = [];
    ...
    function readRedirectGuard() {
        ...
        guardStores().forEach(function (store) {
            try {
                var value = JSON.parse(store.getItem(REDIRECT_GUARD_KEY));
                if (!value || typeof value.target !== 'string') return;

                value.attempts = Number(value.attempts) || 0;
                value.lastAt = Number(value.lastAt) || 0;

                if (!result || value.lastAt > result.lastAt || (value.lastAt === result.lastAt && value.attempts > result.attempts)) {
                    result = value;
                }
            } catch (e) {}
        });
    ...
    function writeRedirectGuard(value) {
        guardStores().forEach(function (store) {
            try {
                store.setItem(REDIRECT_GUARD_KEY, JSON.stringify(value));
            } catch (e) {}
        });
    }
    ...
    function clearRedirectGuard() {
        guardStores().forEach(function (store) {
            try {
                store.removeItem(REDIRECT_GUARD_KEY);
            } catch (e) {}
        });
    }

    function isPersistentRedirectEnabled(value) {
        return value === true || value === 'true' || value === 1 || value === '1';
    }

    function claimRedirectAttempt(targetUrl) {
        var now = Date.now();
        var target = targetHostname(targetUrl) || String(targetUrl);
        var guard = readRedirectGuard();

        if (!guard || guard.target !== target || now < guard.lastAt || now - guard.lastAt > REDIRECT_LOOP_WINDOW_MS) {
            guard = { target: target, attempts: 0, lastAt: now };
        }

        if (guard.attempts >= MAX_REDIRECT_ATTEMPTS) return false;
    ...
    function showStopNotice(message) {
        if (!message || stopNoticeShown) return;
    ...
    function disablePersistentRedirect(message, keepGuard) {
        if (redirectTimerId) clearTimeout(redirectTimerId);
        redirectTimerId = null;

        try {
            if (window.Lampa && Lampa.Storage) Lampa.Storage.set('const_redirect', false);
        } catch (e) {}

        if (!keepGuard) clearRedirectGuard();
        showStopNotice(message);
    }

    function emergencyStopRequested() {
        var hash = String(window.location.hash || '');
        var search = String(window.location.search || '');

        return earlyStopRequested || /(?:^|[#&])no_redirect(?:=(?:1|true))?(?:&|$)/i.test(hash) || /(?:^|[?&])no_redirect=(?:1|true)(?:&|$)/i.test(search);
    }
    ...
    // Працює ще до appready, тому аварійне вимкнення доступне на повільних ТВ.
    window.addEventListener('keydown', function (e) {
        var code = e && (e.keyCode || e.which || e.code);
        if (code === 40 || code === 29461 || (e && e.key === 'ArrowDown')) {
            earlyStopRequested = true;
            disablePersistentRedirect('Постійний редирект скасовано!');
        }
    }, true);
        ...
        try {
            ...
            Lampa.SettingsApi.addParam({
                ...
                field: {
                    ...
                    description: 'Після двох повторних переходів захист сам вимкне редирект. Для ручного скасування затисніть ВНИЗ під час завантаження'
                },
                onChange: function (value) {
                    if (isPersistentRedirectEnabled(value)) {
                        Lampa.Storage.set('const_redirect', true);
                        clearRedirectGuard();
                        stopNoticeShown = false;
                    } else {
                        disablePersistentRedirect();
                    }
                }
            });
            ...
            if (emergencyStopRequested()) {
                disablePersistentRedirect('Постійний редирект скасовано!');
            } else if (isPersistentRedirectEnabled(Lampa.Storage.field('const_redirect'))) {
                var targetServer = Lampa.Storage.get('location_server');
                ...
                if (targetServer && targetServer !== '' && !isAlreadyOnTargetServer(targetServer)) {
                    ...
                    redirectTimerId = setTimeout(function () {
                        ...
                        var currentTarget = Lampa.Storage.get('location_server');
                        if (!isPersistentRedirectEnabled(Lampa.Storage.field('const_redirect')) || currentTarget !== targetServer) return;

                        if (isAlreadyOnTargetServer(targetServer)) {
                            clearRedirectGuard();
                            return;
                        }

                        if (!claimRedirectAttempt(targetServer)) {
                            disablePersistentRedirect('Виявлено цикл редиректів. Постійний редирект вимкнено.', true);
                            return;
                        }

                        navigateTo(targetServer);
                    }, 3000); // 3 секунди на скасування
                } else if (targetServer && isAlreadyOnTargetServer(targetServer)) {
                    clearRedirectGuard();
                }
            }
