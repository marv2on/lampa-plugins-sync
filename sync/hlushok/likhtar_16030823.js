...
(function () {
    ...
    function getLikhtarViewportWidth() {
        var width = 0;
        if (typeof window !== 'undefined' && window.innerWidth) width = window.innerWidth;
        if (!width && typeof document !== 'undefined' && document.documentElement) width = document.documentElement.clientWidth;
        return width || 1920;
    }

    function isLikhtarTouchDevice() {
        try {
    ...
    function getLikhtarDeviceType() {
        var width = getLikhtarViewportWidth();
        var isTouch = isLikhtarTouchDevice();
        if (width <= 820 || (isTouch && width <= 980)) return 'phone';
        if (width <= 1280 || (isTouch && width <= 1400)) return 'tablet';
        if (width >= 1800) return 'tv';
        return 'desktop';
    }

    function getLikhtarHeroHeightEm() {
        var width = getLikhtarViewportWidth();
        var device = getLikhtarDeviceType();
        if (device === 'phone') {
            if (width <= 360) return 12.2;
            if (width <= 412) return 13.2;
            return 14;
        }
        if (device === 'tablet') return 18;
        if (device === 'tv' && width >= 3000) return 27;
        if (device === 'tv') return 24;
        return 22.5;
    }

    function getLikhtarHeroCardWidthVw() {
        var width = getLikhtarViewportWidth();
        var device = getLikhtarDeviceType();
        if (device === 'phone') {
            if (width <= 360) return 96;
            if (width <= 412) return 94;
            return 92;
        }
        if (device === 'tablet') return 74;
        if (device === 'tv' && width >= 3000) return 58;
        if (device === 'tv') return 56;
        return 85;
    }

    function getLikhtarHeroCardMarginRightEm() {
        var width = getLikhtarViewportWidth();
        var device = getLikhtarDeviceType();
        if (device === 'phone') return width <= 412 ? 0.5 : 0.7;
        if (device === 'tablet') return 1.0;
        return 1.5;
    }

    function isLikhtarWifiConnection() {
        try {
            var nav = typeof navigator !== 'undefined' ? navigator : null;
            if (!nav) return false;
            var conn = nav.connection || nav.mozConnection || nav.webkitConnection;
            if (!conn) return false;
            var type = String(conn.type || '').toLowerCase();
            if (type === 'wifi' || type === 'wlan') return true;
            return false;
        } catch (e) {
            return false;
        }
    ...
    function shouldUseLikhtarHeroOriginal() {
        if (getLikhtarDeviceType() !== 'tv') return false;
        // Якщо опція вимкнена — беремо original на TV завжди.
        if (!isSettingEnabled('likhtar_hero_original_wifi', true)) return true;
        return isLikhtarWifiConnection();
    }

    function getLikhtarHeroImageUrl(movie) {
        if (!movie) return '';
        var device = getLikhtarDeviceType();
        var useOriginal = shouldUseLikhtarHeroOriginal();
        if (movie.backdrop_path) {
            // Original лише на TV і лише за правилами налаштування/мережі.
            var backdropSize = useOriginal ? 'original' : (device === 'phone' ? 'w780' : 'w1280');
            return Lampa.TMDB.image('t/p/' + backdropSize + movie.backdrop_path);
        }
        if (movie.poster_path) {
            var posterSize = useOriginal ? 'original' : (device === 'phone' ? 'w500' : 'w780');
            return Lampa.TMDB.image('t/p/' + posterSize + movie.poster_path);
        }
        return '';
    }

    function applyLikhtarWideStyle(items) {
        if (!items || !Lampa.Utils || !Lampa.Utils.extendItemsParams) return;
        // На мобільних не примушуємо формат wide, щоб постери не обрізались.
        if (getLikhtarDeviceType() === 'phone') return;
    ...
    function makeHeroResultItem(movie, heightEm) {
        ...
        return {
            ...
            params: {
                ...
                emit: {
                    onCreate: function () {
                        var img = getLikhtarHeroImageUrl(movie);
                        try {
                            var item = $(this.html);
                ...
                status.onComplite = function () {
                    ...
                    Object.keys(status.data).sort(function (a, b) { return a - b; }).forEach(function (key) {
                        ...
                        if (data && data.results && data.results.length) {
                            ...
                            applyLikhtarWideStyle(data.results);
                            fulldata.push({
                                title: cat.title,
                ...
                return function (callback) {
                    ...
                    network.silent(url, function (json) {
                        ...
                        function build(movies) {
                            ...
                            var heroHeight = getLikhtarHeroHeightEm();
                            var results = moviesWithBackdrop.slice(0, 15).map(function (movie) { return makeHeroResultItem(movie, heroHeight); });
    ...
    function addStyles() {
        ...
        $('body').append(`
            <style id="custom_main_page_css">
                ...
                /* Базова читабельність hero-тексту */
                body.likhtar-plugin .hero-banner .hero-meta,
                body.likhtar-plugin .hero-banner .hero-meta-dynamic {
                    flex-wrap: wrap;
                }
                body.likhtar-plugin .hero-banner .hero-desc {
                    max-width: 65%;
                ...
                @media (max-width: 1280px) {
                    ...
                    body.likhtar-plugin .studios_view .card--wide,
                    body.likhtar-plugin .studios_subscription .card--wide,
                    body.likhtar-plugin .ukrainian_feed .card--wide {
                        width: 14.8em !important;
                    }
                }

                /* Телефони / touch-screen webview */
                @media (max-width: 820px), (hover: none) and (pointer: coarse) and (max-width: 980px) {
                    body.likhtar-plugin .card.hero-banner {
                    ...
                    body.likhtar-plugin .hero-banner .hero-overlay {
                        padding: 0.9em !important;
                        border-radius: 0 0 0.8em 0.8em !important;
                    }
                    body.likhtar-plugin .hero-banner .hero-title {
                        font-size: 1.2em !important;
                        line-height: 1.15 !important;
                        margin-bottom: 0.15em !important;
                    }
                    body.likhtar-plugin .hero-banner .hero-title img {
                    ...
                    body.likhtar-plugin .studios_main .card--wide .card__img,
                    body.likhtar-plugin .studios_view .card--wide .card__img,
                    body.likhtar-plugin .studios_subscription .card--wide .card__img,
                    body.likhtar-plugin .ukrainian_feed .card--wide .card__img {
                        object-fit: contain !important;
                        background: #101010 !important;
                    }
                }

                /* Точна адаптація для 412px (типові сучасні телефони) */
                ...
                /* Точна адаптація для 360px (компактні телефони) */
    ...
    function setupSettings() {
        ...
        Lampa.SettingsApi.addParam({
            component: 'likhtar_plugin',
            param: { name: 'likhtar_hero_original_wifi', type: 'trigger', default: true },
            field: { name: 'Hero 4K: original тільки через Wi-Fi', description: 'Для TV у hero-банері: якщо увімкнено, original завантажується лише через Wi-Fi (коли тип мережі визначено); інакше — завжди.' }
        });
    ...
    function init() {
        ...
        setTimeout(function () {
            ...
            if (heroCard) {
                heroCard.style.width = getLikhtarHeroCardWidthVw() + 'vw';
                heroCard.style.marginRight = getLikhtarHeroCardMarginRightEm() + 'em';
            }
    ...
    function initMarksJacRed() {
        ...
        style.innerHTML = `
            ...
            body.likhtar-plugin img.likhtar-full-logo {
            ...
            @media (max-width: 820px), (hover: none) and (pointer: coarse) and (max-width: 980px) {
                ...
                body.likhtar-plugin .likhtar-poster-badges {
                    top: 0.5em !important;
                    right: 0.1em !important;
                    gap: 0.22em !important;
                }
                body.likhtar-plugin .likhtar-full-badge,
                body.likhtar-plugin .card__mark {
                    font-size: 0.68em !important;
                    padding: 0.24em 0.35em !important;
                }
            }
