(function () {
    ...
    function analyzeContentQualities(movie, activity) {
        ...
        // Отримуємо дані от парсера самостоятельно
        ...
        // Спочатку віддаємо дані з локального кешу між сесіями.
        ...
        if (cachedQualityInfo) {
            if (activity && activity.applecation_quality === undefined) {
                activity.applecation_quality = cachedQualityInfo;
                updateQualityBadges(activity, cachedQualityInfo);
                renderLikhtarFallbackBadges(activity, cachedQualityInfo, movie);
            }
            refreshUnifiedInfoStrip(activity, movie, cachedQualityInfo);
            return;
        }
        ...
        Lampa.Parser.get({
            ...
            if (!results || !results.Results || results.Results.length === 0) {
                const emptyQualityInfo = createEmptyQualityInfo(title, 0);
                if (activity && activity.applecation_quality === undefined) {
                    activity.applecation_quality = emptyQualityInfo;
                    updateQualityBadges(activity, emptyQualityInfo);
                    renderLikhtarFallbackBadges(activity, emptyQualityInfo, movie);
                }
                refreshUnifiedInfoStrip(activity, movie, emptyQualityInfo);
                return;
            }
            ...
            results.Results.forEach((torrent) => {
                ...
                // Аналізуємо название торрента для HDR/DV
            ...
            // Формуємо структурований об'єкт с качеством
            ...
            // Разрешение - беремоо лише максимальное
            ...
            // Аудио - беремоо лише максимальное
            ...
            // Виводимо JSON с результатами (опционально, для отладки)
            // console.log('Applecation Quality:', qualityInfo);

            // Кешуємо підсумок, щоб не робити повторний парсинг після перезапуску.
            qualityBadgesCacheManager.set(movie, qualityInfo);
            
            // Зберігаємо дані в activity для отображения иконок
            if (activity && activity.applecation_quality === undefined) {
                activity.applecation_quality = qualityInfo;
                // Оновлюємо info блок с иконками
                updateQualityBadges(activity, qualityInfo);
                renderLikhtarFallbackBadges(activity, qualityInfo, movie);
            }
            refreshUnifiedInfoStrip(activity, movie, qualityInfo);
            
        }, (error) => {
            // Помилка парсера (не виводимо в консоль, щоб не засмічувати)
        });
    }
...
.applecation-merge-badge--rating {
    background: rgba(19, 29, 56, 0.72);
    border-color: rgba(105, 143, 255, 0.45);
}

.applecation-merge-badge__value {
    font-weight: 700;
}

.applecation-merge-badge__source {
    font-size: 0.86em;
    opacity: 0.78;
    text-transform: uppercase;
}

.applecation-merge-badge--ua {
    background: rgba(26, 126, 228, 0.78);
}

.applecation-merge-badge--en {
    background: rgba(87, 104, 122, 0.78);
}

.applecation-merge-badge--quality {
    background: rgba(131, 43, 189, 0.8);
}

.applecation-merge-badge--dv,
.applecation-merge-badge--hdr {
    background: rgba(42, 47, 57, 0.8);
}

.applecation--merged-inline-ratings .applecation__ratings {
    display: none !important;
}

.applecation--merged-inline-quality .applecation__quality-badges {
    display: none !important;
}
...
.applecation__quality-badges--likhtar {
    display: inline-flex !important;
    align-items: center;
}

.applecation__likhtar-fallback {
    display: inline-flex;
    align-items: center;
    gap: 0.35em;
    flex-wrap: wrap;
...
.applecation__likhtar-fallback .card__mark {
...
body.applecation--no-liquid-glass .applecation .full-person.focus .full-person__photo::after {
    ...
    function isLikhtarPluginLoaded() {
        return !!(window.LIKHTAR_STUDIOS_LOADED || window.LIKHTAR_STUDIOS_VER);
    }

    function mapLikhtarResolutionLabel(value) {
        if (!value) return '';
    ...
    function renderLikhtarFallbackBadges(activity, qualityInfo, movie) {
        if (!isLikhtarPluginLoaded() || !isAlive(activity)) return;
        const badgesContainer = activity.render().find('.applecation__quality-badges');
        if (!badgesContainer.length) return;
        if (badgesContainer.children().length > 0) return;

        const marks = [];
        if (qualityInfo && qualityInfo.ukr) marks.push({ type: 'ua', label: 'UA' });
        if (qualityInfo && qualityInfo.eng) marks.push({ type: 'en', label: 'EN' });

        const resolution = mapLikhtarResolutionLabel(qualityInfo && qualityInfo.quality);
        if (resolution) {
            var resType = 'hd';
            if (resolution === '4K') resType = '4k';
            else if (resolution === '1080p') resType = 'fhd';
            marks.push({ type: resType, label: resolution });
        }

        if (qualityInfo && qualityInfo.hdr) marks.push({ type: 'hdr', label: 'HDR' });

        if (!marks.length) return;

        const html = marks.map((mark) => {
            if (mark.isRating) {
                return `<span class="card__mark card__mark--rating"><span class="mark-star">★</span>${mark.label}</span>`;
            }
            return `<span class="card__mark card__mark--${mark.type}">${mark.label}</span>`;
        }).join('');

        badgesContainer.html(`<span class="applecation__likhtar-fallback">${html}</span>`);
        badgesContainer.addClass('show applecation__quality-badges--likhtar');
    }
    ...
    function collectRatingsForMergeStrip(activity, movie) {
        const result = [];
        const showRatings = Lampa.Storage.get('applecation_show_ratings', false);
        if (!showRatings) return result;

        const pushed = {};
        const order = ['tmdb', 'imdb', 'kp'];
        ...
        // Для зовнішніх плагінів пробуємо прочитати вже намальовані значення з DOM.
        ...
        // Мінімальний резерв — TMDB з картки.
    ...
    function collectQualityForMergeStrip(qualityInfo) {
        const parts = [];
        ...
        const res = mapLikhtarResolutionLabel(qualityInfo.quality);
        if (res) parts.push({ type: 'quality', label: res });

        if (qualityInfo.dv) parts.push({ type: 'dv', label: 'Dolby Vision' });
    ...
    function renderUnifiedInfoStrip(activity, movie, qualityInfo) {
        if (!activity || !isAlive(activity)) return;
        const root = activity.render();
        const strip = activity.render().find('.applecation__merge-strip');
        if (!strip.length) return;

        const ratingParts = collectRatingsForMergeStrip(activity, movie);
        const qualityParts = collectQualityForMergeStrip(qualityInfo);
        const htmlParts = [];

        ratingParts.forEach((r) => {
            htmlParts.push(
                `<span class="applecation-merge-badge applecation-merge-badge--rating"><span class="applecation-merge-badge__value">${r.value}</span><span class="applecation-merge-badge__source">${r.label}</span></span>`
            );
        });
        qualityParts.forEach((q) => {
            htmlParts.push(`<span class="applecation-merge-badge applecation-merge-badge--${q.type}">${q.label}</span>`);
        });

        if (!htmlParts.length) {
            strip.removeClass('show').empty();
            root.removeClass('applecation--merged-inline-ratings applecation--merged-inline-quality');
            return;
        }

        strip.html(htmlParts.join('')).addClass('show');
        if (ratingParts.length) root.addClass('applecation--merged-inline-ratings');
        else root.removeClass('applecation--merged-inline-ratings');

        if (qualityParts.length) root.addClass('applecation--merged-inline-quality');
        else root.removeClass('applecation--merged-inline-quality');
    }

    function refreshUnifiedInfoStrip(activity, movie, qualityInfo) {
        if (!activity || !isAlive(activity)) return;
        ...
        renderUnifiedInfoStrip(activity, resolvedMovie, resolvedQuality);
    }
    ...
    function updateQualityBadges(activity, qualityInfo) {
        ...
        if (badges.length > 0) {
            badgesContainer.removeClass('applecation__quality-badges--likhtar');
            badgesContainer.html(badges.join(''));
            badgesContainer.addClass('show');
        } else {
            badgesContainer.removeClass('show applecation__quality-badges--likhtar');
            badgesContainer.empty();
        }

        refreshUnifiedInfoStrip(activity, activity.applecation_movie, qualityInfo);
    }
    ...
    function loadAndDisplayRatings(activity, movie) {
        ...
        if (!Lampa.Storage.get('applecation_show_ratings', false)) {
            refreshUnifiedInfoStrip(activity, movie, activity.applecation_quality);
            return;
        }
    ...
    function loadLogo(event) {
        ...
        const currentQualityInfo = activity.applecation_quality || createEmptyQualityInfo(data.title || data.name, 0);
        renderLikhtarFallbackBadges(activity, currentQualityInfo, data);
        refreshUnifiedInfoStrip(activity, data, currentQualityInfo);
