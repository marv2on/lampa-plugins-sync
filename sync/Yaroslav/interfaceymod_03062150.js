(function () {
  ...
  Lampa.Lang.add({
    ...
    interface_mod_new_info_panel_style: { en: 'Info panel style', uk: 'Стиль інфо-панелі' },
  ...
  var settings = {
    ...
    info_panel_style: Lampa.Storage.get('interface_mod_new_info_panel_style', 'normal'),
  ...
  (function injectBaseCss() {
    ...
    var css = `
      ...
      *{ -webkit-tap-highlight-color:transparent; -webkit-touch-callout:none; box-sizing:border-box; outline:none; -webkit-user-drag:none; }
      ...
      /* Apple Style for Info Panel */
      .ifx-apple-item {
          display: inline-flex !important; align-items: center; justify-content: center;
          padding: 0 0.8em !important; border-radius: 999px !important;
          height: 2.16em !important; line-height: 2.16em !important;
          margin-right: 0.4em !important; margin-bottom: 0.2em !important;
          font-size: 1.0em !important; font-weight: bold; white-space: nowrap;
          color: rgba(255,255,255,.92) !important; border: 1px solid rgba(255,255,255,.10) !important;
          background: rgba(22,24,30,.28) !important;
  ...
  function initInterfaceModSettingsUI() {
    ...
    Lampa.SettingsApi.addComponent({
      ...
      icon: '<svg viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg"><path d="M4 5c0-.552.448-1 1-1h14c.552 0 1 .448 1 1v2a1 1 0 0 1-1 1H5a1 1 0 0 1-1-1V5Zm0 6c0-.552.448-1 1-1h14c.552 0 1 .448 1 1v2a1 1 0 0 1-1 1H5a1 1 0 0 1-1-1v-2Zm0 6c0-.552.448-1 1-1h14c.552 0 1 .448 1 1v2a1 1 0 0 1-1 1H5a1 1 0 0 1-1-1v-2Zm0 6c0-.552.448-1 1-1h14c.552 0 1 .448 1 1v2a1 1 0 0 1-1 1H5a1 1 0 0 1-1-1v-2Z"/></svg>'
    });
    ...
    add({
      component: 'interface_mod_new',
      param: {
        name: 'interface_mod_new_info_panel_style',
        type: 'select',
        values: {
          'normal': 'Звичайний',
          'apple': 'Apple Style (Розмиття)',
          'apple_lite': 'Apple Lite (Без розмиття)'
        },
        default: 'normal'
      },
      field: { name: Lampa.Lang.translate('interface_mod_new_info_panel_style') }
    });
        ...
        if (typeof key === 'string' && key.indexOf('interface_mod_new_') === 0) {
          switch (key) {
            case 'interface_mod_new_info_panel':
              ...
              break;
            case 'interface_mod_new_info_panel_style':
              settings.info_panel_style = val || 'normal';
              rebuildInfoPanelActive();
              break;
  ...
  function buildInfoPanel(details, movie, isTvShow, originalDetails) {
    ...
    var styleType = Lampa.Storage.get('interface_mod_new_info_panel_style', 'normal');
    var isApple = (styleType === 'apple' || styleType === 'apple_lite');
    var isAppleLite = (styleType === 'apple_lite');
    ...
    function badgeCss(bg, text) {
      if (isApple) return { 'margin-right': '0.4em', 'margin-bottom': '0.2em' };
      if (mono) {
    ...
    function createSpan(txt, cBg, cText, isGenre) {
        var span = $('<span>').text(txt).css(isGenre ? genreCss(cBg, cText) : badgeCss(cBg, cText));
        if (isApple) {
            span.addClass('ifx-apple-item');
            if (isAppleLite) span.addClass('is-lite');
        }
        return span;
    }
    ...
    if (isTvShow && Array.isArray(movie.seasons)) {
      ...
      if (epsText) row1.append(createSpan(epsText, colors.episodes.bg, colors.episodes.text, false));
    }
    ...
    if (isTvShow && movie.next_episode_to_air && movie.next_episode_to_air.air_date) {
      ...
      var txt = diff === 0 ? 'Наступна серія вже сьогодні'
        ...
        : '';
      if (txt) row2.append(createSpan(txt, colors.next.bg, colors.next.text, false));
    }
    ...
    if (!isTvShow && movie.runtime > 0) {
      ...
      if (m > 0) tt += (h > 0 ? ' ' : '') + m + ' хв.';
      row3.append(createSpan(tt, colors.duration.bg, colors.duration.text, false));
    } else if (isTvShow) {
      var avg = calculateAverageEpisodeDuration(movie);
      if (avg > 0) row3.append(createSpan('Тривалість серії ≈ ' + formatDurationMinutes(avg), colors.duration.bg, colors.duration.text, false));
    }
    ...
    if (isTvShow && seasonsCount > 0) {
      row4.append(createSpan('Сезони: ' + seasonsCount, colors.seasons.bg, colors.seasons.text, false));
    }
    ...
    genreList.forEach(function (gn) {
      row4.append(createSpan(gn, '#008080', 'white', true));
    });
