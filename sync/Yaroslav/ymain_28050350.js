(function () {
    ...
    function isLazyLoadEnabled() {
        var v = Lampa.Storage.get('uas_lazy_load');
    ...
    // --- ЛІНИВЕ ЗАВАНТАЖЕННЯ (IntersectionObserver) ---
    var LazyLoader = {
        observer: null,
        init: function() {
    ...
    function fetchLogo(movie, itemElement) {
        ...
        function applyCleanPoster(url) {
            ...
            if (!imgEl.classList.contains('uas-clean-poster')) {
                ...
                setTimeout(() => {
                    ...
                    newImg.onload = () => {
                        ...
                        var parentCard = newImg.closest('.card');
                        if (parentCard) {
                    ...
                    newImg.onerror = () => {
                        if (newImg.dataset.fallbackSrc) {
                            newImg.src = newImg.dataset.fallbackSrc;
                            var parentCard = newImg.closest('.card');
                            if (parentCard) {
                                parentCard.classList.remove('card--preload');
                                parentCard.classList.add('card--loaded');
        ...
        if (langPref === 'off' || (isAlt1 && isHoriz)) {
            ...
            if (!isHoriz && isAlt2) {
                ...
                if (cData && cData !== 'none') applyCleanPoster(cData.split('|')[0]);
                else if (cData === 'none') applyCleanPoster('none');
                else {
                    fetch(PROXIES[0] + getTmdbEndpoint(`${mType}/${movie.id}/images?include_image_language=null`)).then(r=>r.json()).then(res => {
                        if (res.posters && res.posters.length > 0) {
                            ...
                            applyCleanPoster('none');
                        }
                    }).catch(()=>{ applyCleanPoster('none'); });
                }
            }
            return;
        }
                ...
                emit: {
                    onCreate: function () {
                        ...
                        if (isLazyLoadEnabled()) {
                            LazyLoader.add(item[0], function() { fetchLogo(movie, item); });
                        } else {
    ...
    function loadHistoryRow(callback) {
        ...
        if (results.length > 0) {
            callback({ 
                ...
                params: { items: { mapping: 'line', view: limit } } 
            });
        } else {
            callback({ results:[] });
        }
    ...
    async function loadRow(urlId, loadUrl, title, callback) {
        try {
            let limit = getCardsLimit();
            let items = await fetchCatalogPage(loadUrl, limit);
    ...
    async function loadKinobazaRow(urlId, loadUrl, title, callback) {
        try {
            ...
            let items = await fetchKinobazaCatalog(fetchUrl, limit);
            let mapped = items.map(makeWideCardItem);
            callback({ 
                results: mapped, 
                title: '', 
    ...
    async function loadCommunityGemsRow(callback) {
        try {
            let limit = getCardsLimit();
            let listUrl = `https://wh.lme.isroot.in/v2/top?period=7d&top=asc&min_rating=7&per_page=${limit}&page=1`;
    ...
    function createSettings() {
        ...
        Lampa.SettingsApi.addParam({
            component: 'ymainpage',
            param: { name: 'uas_lazy_load', type: 'trigger', default: true },
            field: { name: 'Ліниве завантаження (Lazy Load)', description: 'Завантажувати важкі елементи карток тільки при їх появі на екрані' }
        });

        var limitValues = { '5':'5 карток', '7':'7 карток', '10':'10 карток', '13':'13 карток', '15':'15 карток' };
        Lampa.SettingsApi.addParam({
            component: 'ymainpage',
            param: { name: 'uas_row_cards_count', type: 'select', values: limitValues, default: '10' },
            field: { name: 'Кількість карток головної в ряду', description: 'Не стосується випадкових фільмів та підбірок' }
        });
    ...
    function start() {
        ...
        if (!Lampa.Storage.get('ym_rows_init_v8_fix_8')) {
            ...
            let ll = Lampa.Storage.get('uas_lazy_load');
            if (ll === null || ll === undefined || ll === '') Lampa.Storage.set('uas_lazy_load', true);
        }
        ...
        CardMaker.Card.onVisible = function () {
            ...
            var isStandardVerticalAlt2 = isAlt2 && !isWideCard && !isHistoryCard && !isSpecialCard;

            // Перехоплюємо завантаження постеру, щоб не вантажити стандартний до того як завантажиться чистий (Alt2)
            if (isStandardVerticalAlt2) {
                ...
                if (imgEl && !imgEl.dataset.srcIntercepted) {
                    ...
                    Object.defineProperty(imgEl, 'src', {
                        set: function(val) {
                            this.dataset.fallbackSrc = val;
                        },
                        get: function() {
                            return this.dataset.fallbackSrc || '';
                        },
                        configurable: true
                    });
                }
            ...
            // Миттєво налаштовуємо стилістику та відображення стандартних елементів
            if (useAltDesign) {
            ...
            var doHeavyTasks = function() {
                ...
                if (!isWideCard && !isHistoryCard && !isSpecialCard) {
                    if (isAlt2 && !html.dataset.alt2LogoFetched) {
                        if (data.id) {
                            html.dataset.alt2LogoFetched = 'true';
                            fetchLogo(data, $(html));
                        } else {
                            // Якщо відсутній data.id, примусово використовуємо перехоплений fallback
                            var imgElFb = html.querySelector('.card__img');
                            if (imgElFb && imgElFb.dataset.fallbackSrc && !imgElFb.dataset.fallbackLoaded) {
                                imgElFb.dataset.fallbackLoaded = 'true';
                                delete imgElFb.src;
                                imgElFb.src = imgElFb.dataset.fallbackSrc;
                            }
                        }
                    }
                }
            ...
            // Застосування логіки лінивого завантаження на основі налаштувань
            if (isLazyLoadEnabled()) {
                LazyLoader.add(html, doHeavyTasks);
            } else {
                doHeavyTasks();
            }
