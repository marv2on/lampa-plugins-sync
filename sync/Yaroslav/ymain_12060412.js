(function () {
    ...
    async function getImdbIdForTmdb(tmdbId, type) {
        ...
        try {
            let res = await fetch(getTmdbEndpoint(`${type}/${tmdbId}/external_ids`)).then(r => r.json());
    ...
    async function fetchTmdbWithFallback(type, id) {
        let res = await fetch(getTmdbEndpoint(`${type}/${id}?language=uk`)).then(r=>r.json()).catch(()=>null);
        if (res && (!res.overview || !res.overview.trim())) {
            let enRes = await fetch(getTmdbEndpoint(`${type}/${id}?language=en`)).then(r=>r.json()).catch(()=>null);
    ...
    async function processSingleItem(url) {
        ...
        try {
            let data = await fetch(getTmdbEndpoint(`find/${imdb}?external_source=imdb_id&language=uk`)).then(r => r.json());
            ...
            if (res && (!res.overview || !res.overview.trim())) {
                let enData = await fetch(getTmdbEndpoint(`find/${imdb}?external_source=imdb_id&language=en`)).then(r => r.json());
    ...
    async function searchTmdbByTitleAndYear(title, year, expectedType) {
        ...
        for (let path of endpoints) {
            try {
                let data = await fetch(getTmdbEndpoint(`${path}?query=${encodeURIComponent(title)}&language=uk`)).then(r => r.json());
                if (data && data.results && data.results.length) {
                    ...
                    if (res) {
                        if (!res.overview || !res.overview.trim()) {
                            let enData = await fetch(getTmdbEndpoint(`${path}?query=${encodeURIComponent(title)}&language=en`)).then(r => r.json());
    ...
    function fetchLogo(movie, itemElement) {
        ...
        if (!requireFetch) {
            ...
            fetch(getTmdbEndpoint(`${mType}/${movie.id}/images?include_image_language=uk,en,null`)).then(r => r.json()).then(res => {
            ...
            params: {
                ...
                emit: {
                    onCreate: function () {
                        ...
                        var view = item.find('.card__view').empty();
                        var quality = Lampa.Storage.get('ym_img_quality') || 'w300'; 
                        var imgUrl = Lampa.TMDB.image('t/p/' + quality + movie.backdrop_path);
    ...
    function start() {
        ...
        CardMaker.Card.onVisible = function () {
            ...
            // Нова логіка групування: якщо позиція горизонтальна - додаємо в спільний контейнер через групи (для flex-wrap)
