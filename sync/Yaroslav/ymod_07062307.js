(function () {
    ...
    var styles = `
        ...
        .ymod-ratings-enabled .full-start-new__rate-line.rate-fix,
        .full-start-new__rate-line.rate-fix:empty {
            display: none !important; height: 0px !important; min-height: 0px !important; margin: 0px !important; padding: 0px !important; 
            opacity: 0 !important; visibility: hidden !important; pointer-events: none !important; position: absolute !important;
        }
        ...
        /* Ідеальне, посилене заокруглення для постерів. Кожен кут прописано окремо для гарантії */
        ...
        .ymod-apple-container .custom-rating { font-size: 0.92em !important; }
        .ymod-apple-container .rate--studio { padding: 0 0.8em !important; border-radius: 999px !important; }
        ...
        body.ymod-buttons-apple .full-start-new__button {
            border: 0 !important; background: transparent !important; color: rgba(255,255,255,.92) !important;
            height: 2.8em !important; min-height: 2.8em !important; display: inline-flex !important; align-items: center; justify-content: center;
        ...
        body.ymod-buttons-apple .full-start__button.hidden,
        body.ymod-buttons-apple .full-start-new__button.hidden,
        body.ymod-buttons-apple .full-start__button.hide,
        body.ymod-buttons-apple .full-start-new__button.hide,
        body.ymod-buttons-apple .full-start__button[style*="display: none"],
        body.ymod-buttons-apple .full-start-new__button[style*="display: none"],
        body.ymod-buttons-apple .full-start__button[style*="display:none"],
        body.ymod-buttons-apple .full-start-new__button[style*="display:none"] {
        ...
        div[data-component="ym_logo"], div[data-component="ym_title"], div[data-component="ym_ratings"], div[data-component="ym_ratings_select"], div[data-component="ym_studios"], div[data-component="ym_uator"], div[data-component="ym_buttons"] { display: none !important; }
    `;
    ...
    function handleLogo(e) {
        ...
        if (Lampa.Storage.get("logo_glav", "0") == "1") {
            ...
            if (finalTarget.length) {
                finalTarget.css('font-size', customSize);
            }
            return;
        }
        ...
        function startLogoAnimation(img_url, save_to_cache) {
            ...
            img.onload = function () {
                if (finalTarget.length) {
                    ...
                    // Відключаємо обрізання у батьківського контейнера Lampa
                    ...
                    if (isPoster || isBackdrop) {
                        // Логіка для постерів (вертикальних і горизонтальних)
                        var defaultH;
                        if (isPoster) {
                        ...
                        img.style.maxWidth = "100%"; // Запобігає виходу за межі і гарантує відсутність зрізів
                        ...
                        // Якщо це звичайне лого (текстове)
    ...
    function addRatingBlock(container, className, iconUrl, rawValue, keyName) {
        ...
        var itemClass = isApple ? ' ymod-apple-item' : '';

        var block = $('<div class="full-start__rate custom-rating ' + className + itemClass + '" style="font-size: ' + size + '; order: ' + order + ';">\
    ...
    function handleRatings(e) {
        ...
        if (container.length === 0) {
            ...
            if (cardifyLeft.length > 0) {
                ...
                var localTitle = cardifyLeft.find('.full-start-new__title, .full-start__title').not('.ymod-moved-title').first();
                ...
                var titleLine = render.find('.full-start-new__title, .full-start__title').not('.ymod-moved-title').first();
        ...
        var marginLeftVal = Lampa.Storage.get('omdb_rating_margin_left', '0px');
        var sat = Lampa.Storage.get('omdb_rating_saturation', '75%');
        var gapVal = Lampa.Storage.get('omdb_rating_gap', '0.5em');
        var styleType = Lampa.Storage.get('omdb_rating_style', 'normal');
        var isApple = (styleType === 'apple' || styleType === 'apple_lite');

        container.css({ 'margin-top': marginVal, 'margin-bottom': marginVal, 'margin-left': marginLeftVal });
        
        if (gapVal.indexOf('-') !== -1) {
            container.css({ 'gap': '0px', '--ymod-gap-negative': gapVal }).addClass('ymod-gap-negative');
        } else {
            container.css({ 'gap': gapVal }).removeClass('ymod-gap-negative');
        }
        
        if (isApple) {
            container.addClass('ymod-apple-container');
            if (styleType === 'apple_lite') container.addClass('is-lite');
        } else {
            container.removeClass('ymod-apple-container is-lite');
        }
   ...
   function renderStudiosTitle(render, title, movie) {
        ...
        var mLeft = Lampa.Storage.get("studio_logo_margin_left", "0px");
        ...
        if (movie && movie.production_companies) {
            ...
            companies.forEach(function (co, index) {
                ...
                var itemClass = 'rate--studio studio-logo ymod-studio-item' + (isApple ? ' ymod-apple-item' : '');
        ...
        var containerClass = 'studio-logos-container' + (isApple ? ' ymod-apple-container' + (styleType === 'apple_lite' ? ' is-lite' : '') : '');
        var wrapStyle = isApple ? '' : 'flex-wrap: wrap;';
        
        if (gapEm.indexOf('-') !== -1) {
            wrapStyle += ' gap: 0px; --ymod-gap-negative: ' + gapEm + ';';
            containerClass += ' ymod-gap-negative';
        } else {
            wrapStyle += ' gap: ' + gapEm + ';';
        }

        var wrap = $('<div class="plugin-uk-title-combined" style="margin-left: ' + mLeft + '; margin-top: 10px; margin-bottom: 5px; text-align: left; width: 100%; display: flex; flex-direction: column; align-items: flex-start;"><div class="' + containerClass + '" style="display: flex; align-items: center; ' + wrapStyle + '">' + html + '</div></div>');
        
        var target = render.find(".plugin-hybrid-title");
        if (!target.length) target = render.find(".full-start-new__title, .full-start__title").not('.ymod-moved-title').first();
    ...
    function createSettings() {
        ...
        addSelect('ym_logo', "logo_custom_height", "Обмеження по висоті (vh)", "Відносно висоти екрану", imgHeights, "15vh");
        addSelect('ym_logo', "logo_margin_top", "Відступ зверху (Перенос)", "", extMargins, '0px');
        addSelect('ym_logo', "logo_margin_left", "Відступ зліва (Завжди)", "Діє і для перенесеного і звичайного лого", extMarginsLeft, '0px');
        addSelect('ym_logo', "logo_margin_bottom", "Відступ знизу (Перенос)", "auto (за замовчуванням)", extMargins, 'auto');
        addSelect('ym_logo', "logo_saturation", "Насиченість", "", { "1": "100%", "0.75": "75%", "0.5": "50%", "0.25": "25%", "0": "0% (Ч/Б)" }, "1");
        clearCacheBtn('ym_logo', "Очистити кеш картинок", "img_cache_v3_");
        ...
        addSelect('ym_title', "hybrid_title_mode", "Режим", "", { 'smart': 'Залежно від наявності картинок', 'always_ua': 'Завжди українська' }, 'smart');
        ...
        addSelect('ym_title', "hybrid_title_margin_left", "Відступ зліва", "", extMarginsLeft, '0px');
        clearCacheBtn('ym_title', "Очистити кеш назв", "title_cache_hybrid_v3");

        addStatic(MAIN_C, "ym_buttons_entry", "Кнопки карток", "Зовнішній вигляд кнопок дій", function() { Lampa.Settings.create('ym_buttons'); });
        backTo('ym_buttons', MAIN_C);
        addSelect('ym_buttons', 'ym_button_style', 'Стиль кнопок', 'Зовнішній вигляд кнопок плеєра, закладок тощо', { 'normal': 'Звичайний (Lampa)', 'apple': 'Apple Style (Розмиття)', 'apple_lite': 'Apple Lite (Без розмиття)' }, 'normal', function() { updateBodyClasses(); });
        addSelect('ym_buttons', 'ym_button_size', 'Розмір кнопок', 'Працює для Apple стилів', { '0.8em': '0.8em (Міні)', '0.9em': '0.9em (Дрібні)', '1em': '1em (Стандарт)', '1.1em': '1.1em (Більші)', '1.2em': '1.2em (Великі)', '1.4em': '1.4em (Гігантські)' }, '1em', function() { updateBodyClasses(); });
        addSelect('ym_buttons', 'ym_button_margin_top', 'Відступ зверху (до кнопок)', 'Відстань між деталями фільму та кнопками', extMargins, '1em', function() { updateBodyClasses(); });
        ...
        addSelect('ym_ratings', 'omdb_rating_style', 'Стиль блоку', 'Зовнішній вигляд', { 'normal': 'Звичайний', 'apple': 'Apple Style (Розмиття)', 'apple_lite': 'Apple Lite (Без розмиття)' }, 'normal');
        addSelect('ym_ratings', 'omdb_rating_size', 'Розмір рейтингів', '', extSizes, '1.1em');
        addSelect('ym_ratings', 'omdb_rating_gap', 'Відстань між рейтингами', '', extGaps, '0.5em');
        addSelect('ym_ratings', 'omdb_rating_margin_left', 'Відступ зліва', '', extMarginsLeft, '0px');
        addSelect('ym_ratings', 'omdb_rating_margin', 'Відступ до інших рядків', '', extMargins, '10px');
        ...
        addSelect('ym_studios', 'studio_logo_style', 'Стиль блоку', 'Зовнішній вигляд', { 'normal': 'Звичайний', 'apple': 'Apple Style (Розмиття)', 'apple_lite': 'Apple Lite (Без розмиття)' }, 'normal');
        Lampa.SettingsApi.addParam({ component: 'ym_studios', param: { name: "studio_logo_bg", type: "trigger", default: true }, field: { name: "Підложка", description: "Напівпрозорий фон (не діє для Apple Style)" } });
        addSelect('ym_studios', "studio_logo_size", "Розмір лого", "", extSizes, '0.7em');
        addSelect('ym_studios', "studio_logo_gap", "Відступ між лого", "", extGaps, '0.2em');
        addSelect('ym_studios', "studio_logo_margin_left", "Відступ зліва", "", extMarginsLeft, '0px');
        ...
        addSelect('ym_uator', 'uator_style', 'Стиль блоку', 'Тільки для повної сторінки фільму', { 'normal': 'Звичайний', 'apple': 'Apple Style (Розмиття)', 'apple_lite': 'Apple Lite (Без розмиття)' }, 'normal');
        addSelect('ym_uator', 'uator_gap', 'Відстань між значками', '', extGaps, '0.45em');
        addSelect('ym_uator', 'uator_margin_left', 'Відступ зліва', 'Тільки для повної сторінки', extMarginsLeft, '0px');
        addSelect('ym_uator', 'uator_saturation', 'Насиченість', '', { '100%': '100% (Стандарт)', '75%': '75%', '50%': '50%', '25%': '25%', '0%': '0% (Ч/Б)' }, '100%');
        addSelect('ym_uator', 'uator_rating_size', 'Розмір значків', '', extSizes, '1.1em');
    }
