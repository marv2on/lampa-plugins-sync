(function () {
    ...
    function extractKinobazaItems(html) {
        ...
        doc.querySelectorAll('h4.text-muted.h6.d-inline-block').forEach(h4 => {
            ...
            // Визначаємо тип (серіал чи фільм) за ключовими словами поблизу
        ...
        // Запасний варіант парсингу (якщо попередня структура змінилась)
        if (results.length === 0) {
        ...
        for (let path of endpointsToTry) {
            ...
            try {
                ...
                if (data && data.results && data.results.length > 0) {
                    ...
                    // Якщо рік не збігся, пробуємо знайти точний збіг оригінальної назви (буває Кінобаза ставить не той рік)
                    if (!res) {
        ...
        list: async function (params, oncomplete, onerror) {
            ...
            try {
                ...
                if (isLME) {
                    ...
                    if (page > 1) {
                        return oncomplete({ results:[], page: page, total_pages: 1 });
                    }
                    let listUrl = baseUrl;
                    if (listCache[listUrl]) {
                        mapped = listCache[listUrl];
                    } else {
                        let html = await fetchHtml(listUrl);
                        let items = extractUaserialsCollections(html);
                        mapped = items.map(makeCollectionButtonItem);
                        if (mapped.length > 0) listCache[listUrl] = mapped;
                    }
                    totalPages = 1;
                    ...
                    if (page > 1) {
                        if (listUrl.endsWith('.html')) {
                            listUrl = listUrl.replace('.html', '/page/' + page + '/');
                        } else {
                            listUrl = listUrl.replace(/\/$/, '') + '/page/' + page + '/';
                        }
                    }
                    if (listCache[listUrl]) {
                        mapped = listCache[listUrl];
                    ...
                    let listUrl = page === 1 ? baseUrl : `${baseUrl}page/${page}/`;
                    let items = await fetchCatalogPage(listUrl, 20); 
                    mapped = items; 
                }
