(function () {
    ...
    async function fetchUaFlixCatalog(url, limit, noCache, isMainPage) {
        ...
        if (isMainPage && url.indexOf('page/') === -1) {
            links = links.slice(5);
        }
    ...
    async function loadUaFlixRow(urlId, loadUrl, title, callback) {
        let limit = getCardsLimit();
        try {
            let pageData = await fetchUaFlixCatalog(loadUrl, limit, false, true);
            let results = pageData && pageData.length ? pageData.map(getCardMapFn()) : [];
            callback({ results: results, title: '', source: 'uas_pro_source', uas_content_row: true, params: { items: { mapping: 'line', view: limit } } });
        } catch(e) { 
            callback({ results: [] }); 
    ...
    async function fetchPageData(targetPage, baseUrl, isLME, isKinobazaOnline, isUasCollection, isUasCollectionsList, isUaFlix, params) {
        if (isLME) {
            ...
            let normData = await fetchUaFlixCatalog(targetUrl, 20, false, false);
