(function () {
    ...
    async function fetchHtml(url) {
        for (let proxy of PROXIES) {
            try {
                let res = await fetch(proxy.indexOf('?url=') !== -1 ? proxy + encodeURIComponent(url) : proxy + url);
                if (res.ok) {
                    let text = await res.text();
                    if (text && text.length > 500 && text.indexOf('<html') !== -1 && text.indexOf('just a moment...') === -1) return text;
                }
            } catch (e) {}
        }
        return '';
            ...
            try {
                ...
                if (data && data.results && data.results.length) {
                    let res = data.results.find(r => {
                        if (expectedType && r.media_type && r.media_type !== expectedType && path.indexOf('search/multi') !== -1) return false;
                        let rYear = (r.release_date || r.first_air_date || '').substring(0, 4);
                        return rYear === year || rYear === String(parseInt(year)-1) || rYear === String(parseInt(year)+1);
                    ...
                    if (res) {
                        if (!res.overview || !res.overview.trim()) {
                            ...
                            let enRes = enData.results ? enData.results.find(x => x.id === res.id) : null;
                            if (enRes && enRes.overview) res.overview = enRes.overview;
                        }
    ...
    async function fetchCatalogPage(url, limit) {
        ...
        let unique = {}, finalItems = tmdbItems.filter(i => i && i.id && i.backdrop_path && !unique[i.id] && (unique[i.id] = true));
    ...
    async function fetchKinobazaCatalog(url, limit, noCache) {
        ...
        let tmdbItems = await processInQueue(items, async i => searchTmdbByTitleAndYear(i.title, i.year, i.type), 5);
        let unique = {}, finalItems = tmdbItems.filter(i => i && i.id && i.backdrop_path && !unique[i.id] && (unique[i.id] = true));
    ...
    async function getLmeTmdbItems(items) {
        var promises = items.map(async item => {
            ...
            return tmdbData && !tmdbData.error && tmdbData.backdrop_path ? Object.assign({}, tmdbData, { media_type: type }) : null;
        });
    ...
    async function loadRow(urlId, loadUrl, title, callback) {
        try {
            ...
            callback({ results: pageData.map(makeWideCardItem), title: '', source: 'uas_pro_source', uas_content_row: true, params: { items: { mapping: 'line', view: limit } } });
    ...
    async function loadCommunityGemsRow(callback) {
        try {
            ...
            callback({ results: mappedData.map(makeWideCardItem), title: '', source: 'uas_pro_source', uas_content_row: true, params: { items: { mapping: 'line', view: limit } } });
        } catch(e) { callback({ results:[] }); }
    }
    ...
    async function loadRandomMoviesRow(callback) {
        try {
            ...
            callback({ results: randData.map(makeWideCardItem), title: '', uas_content_row: true, params: { items: { mapping: 'line', view: 5 } } });
        } catch(e) { callback({ results:[] }); }
    }
            ...
            try {
                ...
                if (page === 1) {
                    let r = await Promise.all([
                        fetchPageData(1, baseUrl, isLME, isKinobazaOnline, isUasCollection, isUasCollectionsList, params),
                        fetchPageData(2, baseUrl, isLME, isKinobazaOnline, isUasCollection, isUasCollectionsList, params)
                    ]);
                    res = { mapped: r[0].mapped.concat(r[1].mapped), total: r[0].total };
                } else {
                    res = await fetchPageData(page + 1, baseUrl, isLME, isKinobazaOnline, isUasCollection, isUasCollectionsList, params);
                }
    ...
    function start() {
        ...
        if (!Lampa.Storage.get('ym_rows_init_v8_fix_8')) {
            ...
            DEFAULT_ROWS_SETTINGS.forEach(function(r) { if (Lampa.Storage.get(r.id) === null) Lampa.Storage.set(r.id, r.default); });
