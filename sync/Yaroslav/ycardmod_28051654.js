(function () {
		...
		_createClass(Player,[
                    ...
                    this.youtube = new window.YT.Player(
                        this.html.find(".cardify-trailer__youtube-iframe")[0],
                        {
                            ...
                            playerVars: {
                                ...
                                mute: 1,
                                start: 8 // Завжди старт з 8-ї секунди
                            },
            ...
            },
            {
                ...
                value: function initHtml5() {
                    ...
                    videoElem.controls = false; // Суворо вимикаємо контроли
                    videoElem.disablePictureInPicture = true;
                    videoElem.disableRemotePlayback = true;

                    if (this.isBgMode) videoElem.muted = true;
                    
                    // Стилізуємо щоб приховати всі можливі системні UI браузера
                    videoElem.style.width = '100%';
                    videoElem.style.height = '100%';
                    videoElem.style.objectFit = 'cover';
                    videoElem.style.border = 'none';
                    videoElem.style.pointerEvents = 'none'; // Ігнорує кліки (не викличе плей/паузу)
                    videoElem.style.outline = 'none';
                    videoElem.style.background = 'transparent';
                    videoElem.tabIndex = -1; // Прибираємо з фокусу

                    if (blurVal > 0) videoElem.style.filter = 'blur(' + blurVal + 'px)';
                    if (scale > 1) videoElem.style.transform = 'scale(' + scale + ')';

                    // Додаємо Media Fragments для 8 сек
                    ...
                    // Надійно ставимо час, як тільки відома тривалість
			...
			},
			{
				...
				value: function skodf(e) {
                    ...
                    if (trailerBtn.length) {
                        ...
                        // ДОДАЄМО КНОПКУ "IMDB TRAILER" В ІНТЕРФЕЙС
                        if (render.find('.view--imdb_trailer').length === 0) {
                            var imdbBtn = $('<div class="full-start__button selector view--imdb_trailer"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="5 3 19 12 5 21 5 3"></polygon></svg><span>IMDB Trailer</span></div>');
                                ...
                                $.ajax({
                                    ...
                                    success: function(data) {
                                        if (data && data.trailer && data.trailer.playback) {
                                            ...
                                            // Шукаємо автоматично найкращу якість
                                            var video_url = p['auto'] || p['1080'] || p['720'] || p['480'] || p['sd'];
                                            if (!video_url) {
                                            ...
                                            if (video_url) {
                                                ...
                                                // Паузимо фоновий трейлер і запускаємо основний плеєр Лампи
                                                if (window.cardifyBgPlayer && window.cardifyBgPlayer.pause) {
                                                    window.cardifyBgPlayer.pause();
                                                }
                            ...
                            trailerBtn.after(imdbBtn); // Додаємо після стандартної кнопки
                        }
                    }
	...
	function startPlugin() {
        ...
        // ПАУЗА ФОНОВОГО ТРЕЙЛЕРА ПРИ ВІДКРИТТІ ОСНОВНОГО ПЛЕЄРА LAMPA
        if (window.Lampa && Lampa.Player && Lampa.Player.listener) {
            Lampa.Player.listener.follow('ready', function () {
                if (window.cardifyBgPlayer && typeof window.cardifyBgPlayer.pause === 'function') {
                    window.cardifyBgPlayer.pause();
                }
            });

            Lampa.Player.listener.follow('destroy', function () {
                if (window.cardifyBgPlayer && typeof window.cardifyBgPlayer.play === 'function') {
		...
		Follow.get(Type.de([102, 117, 108, 108]), function (e) {
			if (Type.co(e)) {
                ...
                var use_proxy = Main.cases()[Main.stor()].field("cardify_trailer_proxy") !== false; // За замовчуванням увімкнено
                ...
                var finalizeTrailer = function(tr) {
                    ...
                    processSlideshow(); // Запускаємо слайдшоу як фон (перевірки безпеки виконуються всередині processSlideshow)
                };
                            ...
                            $.ajax({
                                ...
                                success: function (data) {
                                    ...
                                    if (data && data.trailer && data.trailer.playback) {
                                        ...
                                        var video_url = null;
                                        // 1. Пошук вибраної або нижчої якості
                                        for (var i = startIndex; i < order.length; i++) {
                                            if (p[order[i]]) { video_url = p[order[i]]; break; }
                                        }
                                        // 2. Якщо не знайдено, пошук будь-якої доступної
                                        if (!video_url) {
                                            for (var i = 0; i < order.length; i++) {
                                                if (p[order[i]]) { video_url = p[order[i]]; break; }
                                            }
                                        }
                                        // 3. Якщо і так нічого немає, беремо перше доступне
                                        if (!video_url) {
                                            var keys = Object.keys(p);
                                            if (keys.length > 0) video_url = p[keys[0]];
                                        }
                                        ...
                                        if (video_url) {
                                            tr = {
                                                ...
                                                id: imdb_id,
                                                startTime: 10 // Завжди старт з 8-ї секунди
                                            };
                                        }
                                    }
                                    ...
                                    if (tr) {
                                        ...
                                        // СУВОРО БЕЗ YOUTUBE: Якщо немає трейлера в IMDB, перемикаємось на Слайдшоу
                                        isBgTrailers = false;
                                        processSlideshow();
                                    }
                                },
                                error: function () {
                                    // СУВОРО БЕЗ YOUTUBE: Якщо помилка API, перемикаємось на Слайдшоу
                                    isBgTrailers = false;
                                    processSlideshow();
                                }
                            });
                        } else {
                            // СУВОРО БЕЗ YOUTUBE: Якщо немає IMDB ID, перемикаємось на Слайдшоу
                            isBgTrailers = false;
                            processSlideshow();
                        }
                    } else {
                        // TMDB варіант - отримуємо відео з YouTube
