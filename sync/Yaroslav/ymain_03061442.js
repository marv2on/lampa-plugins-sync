(function () {
    ...
    function updateDynamicStyles() {
        ...
        document.body.classList.toggle('uas-alt-design-active', altType === '1' || altType === '2');
        document.body.classList.toggle('uas-alt-design-2', altType === '2');

        document.body.style.setProperty('--uas-badge-size', (Lampa.Storage.get('uas_alt_badge_size') || '0.7') + 'em');
        document.body.classList.toggle('uas-hide-text', Lampa.Storage.get('uas_text_hide') === true);
        ...
        var bgMode = Lampa.Storage.get('uas_bg_mode') || '1';
        document.body.classList.toggle('uas-bg-disabled', bgMode === '0');
        document.body.classList.toggle('uas-bg-instant', bgMode === '2');
    }
    ...
    var BgManager = {
        ...
        change: function(url, instant) {
            if ((Lampa.Storage.get('uas_bg_mode') || '1') === '0' || !url || this.activeUrl === url) return;
            clearTimeout(this.timer);
            var execute = () => {
                this.activeUrl = url;
                var nextLayer = this.currentLayer === 1 ? this.layer2 : this.layer1;
                var activeLayer = this.currentLayer === 1 ? this.layer1 : this.layer2;
                nextLayer.style.backgroundImage = 'url(' + url + ')';
                nextLayer.classList.add('active'); activeLayer.classList.remove('active');
                this.currentLayer = this.currentLayer === 1 ? 2 : 1;
            };
            instant ? execute() : (this.timer = setTimeout(execute, 0));
        },
    ...
    var requestQueue = {
        ...
        process: function () {
            while (this.activeCount < this.maxParallel && this.queue.length) {
                ...
                Promise.resolve().then(task).catch(()=>{})["finally"](() => { this.activeCount--; this.process(); });
            }
        }
    };
    ...
    async function fetchHtml(url) {
        for (let proxy of PROXIES) {
            try {
                let res = await fetch(proxy.includes('?url=') ? proxy + encodeURIComponent(url) : proxy + url);
                if (res.ok) {
                    let text = await res.text();
                    if (text && text.length > 500 && text.includes('<html') && !text.includes('just a moment...')) return text;
                }
            } catch (e) {}
        }
        return '';
    ...
    function formatExtRating(val, key) {
        ...
        else if (['mc', 'rt', 'mdblist', 'popcorn', 'trakt'].includes(key) && num > 10) num /= 10;
    ...
    function renderSeasonBadge(cardHtml, tmdbData, targetContainer) {
        ...
        var currentSeason = tmdbData.seasons.find(s => s.season_number === last.season_number);
    ...
    function extractUaserialsCollections(html) {
        ...
        doc.querySelectorAll('a[href*="/collections/"]').forEach(a => {
            ...
            if (href && href.match(/\/collections\/\d+/) && !href.includes('/page/')) {
                ...
                if (!title && a.closest('.short, .collection-item, article')) {
    ...
    async function getImdbId(url) {
        ...
        let id = (await fetchHtml(url)).match(/imdb\.com\/title\/(tt\d+)/i);
    ...
    async function processSingleItem(url) {
        ...
        try {
            ...
            let res = data.movie_results?.length ? { ...data.movie_results[0], media_type: 'movie' } : data.tv_results?.length ? { ...data.tv_results[0], media_type: 'tv' } : null;
            if (res && (!res.overview || !res.overview.trim())) {
                let enData = await fetch(PROXIES[0] + getTmdbEndpoint(`find/${imdb}?external_source=imdb_id&language=en`)).then(r => r.json());
                let enRes = enData.movie_results?.length ? enData.movie_results[0] : enData.tv_results?.length ? enData.tv_results[0] : null;
                if (enRes?.overview) res.overview = enRes.overview;
            }
    ...
    async function searchTmdbByTitleAndYear(title, year, expectedType) {
        ...
        for (let path of endpoints) {
            try {
                let data = await fetch(PROXIES[0] + getTmdbEndpoint(`${path}?query=${encodeURIComponent(title)}&language=uk`)).then(r => r.json());
                if (data?.results?.length) {
                    let res = data.results.find(r => {
                        ...
                        return rYear === year || rYear === String(year-1) || rYear === String(parseInt(year)+1);
                    }) || data.results.find(r => (!expectedType || r.media_type === expectedType) && (r.original_title || r.original_name || '').toLowerCase() === title.toLowerCase());
                    
                    if (res) {
                        if (!res.overview?.trim()) {
                            let enData = await fetch(PROXIES[0] + getTmdbEndpoint(`${path}?query=${encodeURIComponent(title)}&language=en`)).then(r => r.json());
                            let enRes = enData.results?.find(x => x.id === res.id);
                            if (enRes?.overview) res.overview = enRes.overview;
                        }
                        res.media_type = res.media_type || expectedType || (res.first_air_date ? 'tv' : 'movie');
                        tmdbItemCache[cacheKey] = res; return res;
                    }
                }
            } catch(e) {}
        } return null;
    ...
    async function fetchCatalogPage(url, limit = 10) {
        if (listCache[url]) return listCache[url];
        let tmdbItems = await processInQueue(extractItemLinks(await fetchHtml(url)).slice(0, limit), processSingleItem, 5);
        let unique = {}, finalItems = tmdbItems.filter(i => i?.id && i.backdrop_path && !unique[i.id] && (unique[i.id] = true));
    ...
    async function fetchKinobazaCatalog(url, limit, noCache = false) {
        ...
        let tmdbItems = await processInQueue(extractKinobazaItems(await fetchHtml(url)), async i => searchTmdbByTitleAndYear(i.title, i.year, i.type), 5);
        let unique = {}, finalItems = tmdbItems.filter(i => i?.id && i.backdrop_path && !unique[i.id] && (unique[i.id] = true));
    ...
    async function getLmeTmdbItems(items) {
        return (await Promise.all(items.map(async item => {
            if (!item) return null;
            let type, id;
            if (typeof item.id === 'string' && item.id.includes(':')) [type, id] = item.id.split(':');
            ...
            return tmdbData && !tmdbData.error && tmdbData.backdrop_path ? { ...tmdbData, media_type: type } : null;
        }))).filter(Boolean);
    }

    // --- ОПТИМІЗОВАНИЙ fetchLogo ---
    function fetchLogo(movie, itemElement) {
        ...
        function applyCleanPoster(url) {
            ...
            var tmp = new Image();
            tmp.onload = function() {
                delete imgEl.src; // вбиваємо інтерцептор, якщо він ще є
                imgEl.src = cleanUrl;
        ...
        function renderTextLogo() {
            ...
            if (!textLogo) {
                ...
                textLogo.innerText = (langPref.includes('en') ? (movie.original_title || movie.original_name) : null) || movie.title || movie.name;
        ...
        if (!needsLogo && !needsCleanPoster) {
            ...
            return; // Жодних мережевих запитів, якщо лого та чистий постер вимкнені
        }
        ...
        var requireFetch = (needsLogo && !cachedLogo && !langPref.startsWith('text_')) || (needsCleanPoster && !cachedPosterData);

        if (!requireFetch) {
            if (needsCleanPoster) applyCleanPoster(pPath || 'none');
            if (needsLogo) {
                if (langPref.startsWith('text_')) renderTextLogo();
            ...
            fetch(PROXIES[0] + getTmdbEndpoint(`${mType}/${movie.id}/images?include_image_language=uk,en,null`)).then(r => r.json()).then(res => {
                ...
                if (needsLogo) {
                    var found = (langPref === 'uk' ? res.logos.find(l => l.iso_639_1 === 'uk') : langPref === 'en' ? res.logos.find(l => l.iso_639_1 === 'en') : res.logos.find(l => l.iso_639_1 === 'uk') || res.logos.find(l => l.iso_639_1 === 'en'));
                ...
                if (needsLogo) {
                    ...
                    else if (langPref.startsWith('text_')) renderTextLogo();
                    else renderLogo(finalLogo);
                }
            }).catch(() => {
                if (needsLogo) { Cache7Days.set(cacheKey, 'none'); langPref.startsWith('text_') ? renderTextLogo() : renderLogo('none'); }
            ...
            params: {
                ...
                emit: {
                    onCreate: function () {
                        ...
                        var imgUrl = imgUrlPath ? PROXIES[0] + Lampa.TMDB.image('t/p/' + (Lampa.Storage.get('ym_img_quality') || 'w300') + imgUrlPath) : '';
            ...
            params: {
                ...
                emit: {
                    onCreate: function () {
                        ...
                        var view = item.find('.card__view').empty(); 
                        var imgUrl = PROXIES[0] + Lampa.TMDB.image('t/p/' + (Lampa.Storage.get('ym_img_quality') || 'w300') + movie.backdrop_path);
                        view.css({ 'background-image': `url(${imgUrl})`, 'background-size': 'cover', 'background-position': 'center', 'padding-bottom': '56.25%', 'height': '0', 'position': 'relative' });
                        view.append('<div class="card-backdrop-overlay"></div>');
    ...
    function loadHistoryRow(callback) {
        let hist =[], allFavs = {};
        try { if (Lampa.Favorite?.all) { allFavs = Lampa.Favorite.all() || {}; hist = allFavs.history || []; } } catch(e) {}
        
        let results =[], getRandImg = (arr) => {
            let valid = (arr || []).filter(i => i?.backdrop_path || i?.poster_path);
            if (!valid.length) return { img:'', bg:'' };
            let r = valid[Math.floor(Math.random() * valid.length)], path = r.backdrop_path || r.poster_path;
            return { img: PROXIES[0] + Lampa.TMDB.image('t/p/' + (Lampa.Storage.get('ym_img_quality') || 'w300') + path), bg: PROXIES[0] + Lampa.TMDB.image('t/p/w300' + path) };
        };
        ...
        if (hist.length) {
            let unique = {};
            results = results.concat(hist.filter(h => h?.id && (h.title || h.name) && !unique[h.id] && (unique[h.id] = true)).slice(0, getCardsLimit()).map(makeHistoryCardItem));
        }
    ...
    async function loadRow(urlId, loadUrl, title, callback) {
        try {
            ...
            callback({ results: (await fetchCatalogPage(loadUrl, limit)).map(makeWideCardItem), title: '', source: 'uas_pro_source', uas_content_row: true, params: { items: { mapping: 'line', view: limit } } });
    ...
    async function loadKinobazaRow(urlId, loadUrl, title, callback) {
        try {
            let limit = getCardsLimit();
            callback({ results: (await fetchKinobazaCatalog(loadUrl + '1', limit)).map(makeWideCardItem), title: '', source: 'uas_pro_source', uas_content_row: true, params: { items: { mapping: 'line', view: limit } } });
        } catch(e) { callback({ results:[] }); }
    }
    ...
    async function loadCommunityGemsRow(callback) {
        try {
            ...
            callback({ results: (await getLmeTmdbItems(Array.isArray(res) ? res : (res.items ||[]))).map(makeWideCardItem), title: '', source: 'uas_pro_source', uas_content_row: true, params: { items: { mapping: 'line', view: limit } } });
        } catch(e) { callback({ results:[] }); }
    }
    ...
    async function loadRandomMoviesRow(callback) {
        try {
            ...
            callback({ results: (await fetchKinobazaCatalog(url, 5, true)).map(makeWideCardItem), title: '', uas_content_row: true, params: { items: { mapping: 'line', view: 5 } } });
        } catch(e) { callback({ results:[] }); }
    }
    ...
    async function fetchPageData(targetPage, baseUrl, isLME, isKinobazaOnline, isUasCollection, isUasCollectionsList, params) {
        if (isLME) {
            ...
            return { mapped: await getLmeTmdbItems(Array.isArray(res) ? res : (res.items ||[])), total: res.total_pages || 10 };
            ...
            return { mapped: await fetchKinobazaCatalog(baseUrl + targetPage, 30), total: 50 };
        } else {
            return { mapped: await fetchCatalogPage(targetPage === 1 ? baseUrl : `${baseUrl}page/${targetPage}/`, 20), total: 50 };
        }
    }

    Lampa.Api.sources.uas_pro_source = {
        list: async function (params, oncomplete, onerror) {
            let page = params.page || 1, baseUrl = '', isLME = false, isKinobazaOnline = false, isUasCollection = params.is_uas_collection, isUasCollectionsList = false;
            ...
            try {
                let res = page === 1 ? await Promise.all([fetchPageData(1, baseUrl, isLME, isKinobazaOnline, isUasCollection, isUasCollectionsList, params), fetchPageData(2, baseUrl, isLME, isKinobazaOnline, isUasCollection, isUasCollectionsList, params)]).then(r => ({ mapped: r[0].mapped.concat(r[1].mapped), total: r[0].total })) : await fetchPageData(page + 1, baseUrl, isLME, isKinobazaOnline, isUasCollection, isUasCollectionsList, params);
                if (page > 1) hideLoadingToast();
                res.mapped.length > 0 ? oncomplete({ results: res.mapped, page: page, total_pages: res.total }) : onerror();
    ...
    function createSettings() {
        ...
        DEFAULT_ROWS_SETTINGS.forEach(r => {
            ...
            Lampa.SettingsApi.addParam({ component: 'ymainpage_rows', param: { name: r.id + '_order', type: 'select', values: { '1': 'Позиція 1', '2': 'Позиція 2', '3': 'Позиція 3', '4': 'Позиція 4', '5': 'Позиція 5', '6': 'Позиція 6', '7': 'Позиція 7', '8': 'Позиція 8', '9': 'Позиція 9' }, default: r.defOrder }, field: { name: 'Порядок: ' + r.title, description: 'Яким по рахунку виводити цей рядок' } });
        });
        ...
        Lampa.SettingsApi.addParam({ component: 'ymainpage', param: { name: 'uas_alt_badge_pos', type: 'select', values: { 'tl': 'Верхній Лівий', 'tr': 'Верхній Правий', 'ml': 'Середина Лівий', 'mr': 'Середина Правий', 'bl': 'Нижній Лівий', 'br': 'Нижній Правий' }, default: 'tl' }, field: { name: 'Позиція бейджів (Рік, Сезони, UA)', description: '' } });
        Lampa.SettingsApi.addParam({ component: 'ymainpage', param: { name: 'uas_alt_rating_pos', type: 'select', values: { 'tl': 'Верхній Лівий', 'tr': 'Верхній Правий', 'ml': 'Середина Лівий', 'mr': 'Середина Правий', 'bl': 'Нижній Лівий', 'br': 'Нижній Правий' }, default: 'tr' }, field: { name: 'Позиція рейтингу', description: '' } });
        ...
        ['tmdb', 'imdb', 'rt', 'mc', 'letterboxd', 'trakt', 'mdblist', 'popcorn'].forEach(k => Lampa.SettingsApi.addParam({ component: 'ymainpage_ext_rt', param: { name: 'uas_ext_rt_'+k, type: 'trigger', default: true }, field: { name: k.toUpperCase(), description: '' } }));

        Lampa.Settings.listener.follow('open', e => {
            if (e.name === 'ymainpage') {
                e.body.find('[data-name="uas_support_yarik"]').on('hover:enter', () => window.open('https://lampalampa.free.nf/', '_blank'));
                e.body.find('[data-name="uas_support_lme"]').on('hover:enter', () => window.open('https://lampame.github.io/main/#uk', '_blank'));
                e.body.find('[data-name="uas_rows_menu_btn"]').on('hover:enter', () => Lampa.Settings.create('ymainpage_rows'));
                e.body.find('[data-name="uas_ext_rt_menu_btn"]').on('hover:enter', () => Lampa.Settings.create('ymainpage_ext_rt'));
                e.body.find('[data-name="uas_pro_tmdb_btn"]').on('hover:enter', () => Lampa.Input.edit({ title: 'TMDB API', value: Lampa.Storage.get('uas_pro_tmdb_apikey') || '', free: true, nosave: true }, v => { if (v !== undefined) { Lampa.Storage.set('uas_pro_tmdb_apikey', v.trim()); Lampa.Noty.show('Збережено.'); } }));
                e.body.find('[data-name="uas_alt_apikey_btn"]').on('hover:enter', () => Lampa.Input.edit({ title: 'API Ключ easyratingsdb', value: Lampa.Storage.get('uas_alt_design_apikey') || '', free: true, nosave: true }, v => { if (v !== undefined) { Lampa.Storage.set('uas_alt_design_apikey', v.trim()); Lampa.Noty.show('Збережено.'); } }));
            } else if (e.name === 'ymainpage_ext_rt') {
                e.body.find('[data-name="uas_ext_rt_back"]').on('hover:enter', () => Lampa.Settings.create('ymainpage'));
                e.body.find('[data-name="uas_omdb_key_btn"]').on('hover:enter', () => Lampa.Input.edit({ title: 'OMDB', value: Lampa.Storage.get('uas_omdb_api_key') || '', free: true, nosave: true }, v => { if(v!==undefined){ Lampa.Storage.set('uas_omdb_api_key', v.trim()); Lampa.Noty.show('Збережено.'); } }));
                e.body.find('[data-name="uas_mdblist_key_btn"]').on('hover:enter', () => Lampa.Input.edit({ title: 'MDBList', value: Lampa.Storage.get('uas_mdblist_api_key') || '', free: true, nosave: true }, v => { if(v!==undefined){ Lampa.Storage.set('uas_mdblist_api_key', v.trim()); Lampa.Noty.show('Збережено.'); } }));
            } else if (e.name === 'ymainpage_rows') {
                e.body.find('[data-name="uas_rows_back"]').on('hover:enter', () => Lampa.Settings.create('ymainpage'));
            }
        });

        Lampa.Settings.listener.follow('change', e => { if (['uas_alt_design_type', 'uas_alt_badge_size', 'uas_text_hide', 'uas_text_align', 'uas_text_desc_align', 'uas_text_title_size', 'uas_text_desc_size', 'uas_bg_mode'].includes(e.name)) updateDynamicStyles(); });
    }
    ...
    function overrideApi() {
        Lampa.Api.sources.tmdb.main = function (params, oncomplite, onerror) {
            var rowDefs =[
                ...
                { id: 'ym_row_kinobaza', defOrder: 5, type: 'kinobaza', url: 'kinobaza_streaming', loadUrl: 'https://kinobaza.com.ua/online?order_by=date_desc&rating=1&rating_max=10&imdb_rating=1&imdb_rating_max=10&per_page=30&translated=has_ukr_audio&page=', title: 'Новинки Стрімінгів UA', icon: 'https://upload.wikimedia.org/wikipedia/commons/a/a5/Netflix_meaningful_logo.svg' },
            ...
            for (let def of rowDefs) {
                ...
                if (enabled === null || enabled === undefined || enabled === '') enabled = DEFAULT_ROWS_SETTINGS.find(r => r.id === def.id)?.default ?? true;
                else enabled = (enabled === true || enabled === 'true');
                if (enabled) activeRows.push({ ...def, order: parseInt(Lampa.Storage.get(def.id + '_order')) || def.defOrder });
            }
            activeRows.sort((a, b) => a.order - b.order);
            
            let parts_data =[];
            activeRows.forEach(def => {
                if (def.type !== 'history') parts_data.push(cb => cb({ results:[makeTitleButtonItem(def.title, def.url, def.icon)], title: '', uas_title_row: true, params: { items: { mapping: 'line', view: 1 } } }));
                parts_data.push(cb => {
            ...
            if(!parts_data.length) parts_data.push(cb => loadRow('uas_movies_new', 'https://uaserials.com/films/p/', 'Новинки фільмів', cb));
    ...
    function start() {
        ...
        if (!Lampa.Storage.get('ym_rows_init_v8_fix_8')) {
            ...
            DEFAULT_ROWS_SETTINGS.forEach(r => { if (Lampa.Storage.get(r.id) === null) Lampa.Storage.set(r.id, r.default); });
            ['uas_show_flag','uas_show_fav_card','uas_show_history_btn','uas_badge_year','uas_badge_season','uas_badge_rating','uas_lazy_load'].forEach(k => { if(Lampa.Storage.get(k) === null) Lampa.Storage.set(k, true); });
        ...
        style.innerHTML = `
            ...
            .card .card-custom-badges { position: absolute !important; display: flex !important; flex-direction: column !important; gap: 0.2em !important; z-index: 20 !important; pointer-events: none !important; background: transparent !important; }
            .card .badge-pos-tl { left: 0.3em !important; top: 0.55em !important; align-items: flex-start !important; } .card .badge-pos-tr { right: 0.3em !important; top: 0.55em !important; align-items: flex-end !important; }
            .card .badge-pos-ml { left: 0.3em !important; top: 50% !important; transform: translateY(-50%) !important; align-items: flex-start !important; } .card .badge-pos-mr { right: 0.3em !important; top: 50% !important; transform: translateY(-50%) !important; align-items: flex-end !important; }
            .card .badge-pos-bl { left: 0.3em !important; bottom: 0.55em !important; align-items: flex-start !important; } .card .badge-pos-br { right: 0.3em !important; bottom: 0.55em !important; align-items: flex-end !important; }
            .card .card-custom-badges > div { position: static !important; margin: 0 !important; height: 1.8em !important; min-height: 1.8em !important; width: auto !important; border-radius: 2em !important; display: flex !important; align-items: center !important; justify-content: center !important; background: rgba(0, 0, 0, 0.55) !important; padding: 0 0.6em !important; font-size: var(--uas-badge-size, 0.7em) !important; font-weight: 700 !important; color: #fff !important; box-shadow: none !important; line-height: 1 !important; border: 1px solid rgba(255,255,255,0.15) !important; box-sizing: border-box !important; }
            ...
            .card .badge-pos-bl .ext-rating-badge, .card .badge-pos-br .ext-rating-badge { order: 0 !important; } .card .badge-pos-bl .card__ua_flag, .card .badge-pos-br .card__ua_flag { order: 1 !important; } .card .badge-pos-bl .card__type, .card .badge-pos-br .card__type { order: 2 !important; } .card .badge-pos-bl .card-badge-age,.card .badge-pos-br .card-badge-age{ order: 3 !important; }
            .card .badge-pos-ml .card__ua_flag, .card .badge-pos-mr .card__ua_flag { order: 1 !important; } .card .badge-pos-ml .card-badge-age,.card .badge-pos-mr .card-badge-age{ order: 2 !important; } .card .badge-pos-ml .card__type, .card .badge-pos-mr .card__type { order: 3 !important; }
            ...
            @media (orientation: portrait), (max-width: 768px) {
                .card--wide-custom, .card--history-custom { width: 14em !important; } .card--collection-btn { width: 14em !important; height: auto !important; aspect-ratio: 16/9; }
                .card--wide-custom .custom-overview-bottom { display: none !important; } .card--wide-custom .custom-title-bottom { font-size: 1em !important; margin-top: 0.1em; }
                .items-line[data-uas-title-row="true"] { margin-bottom: 0 !important; } .items-line[data-uas-content-row="true"] { margin-bottom: 0.2em !important; }
                .card--title-btn { margin: 0 !important; padding: 0.2em 1em !important; min-height: 2em !important; } .title-btn-text { font-size: 1.1em !important; }
            }
        `;
        document.head.appendChild(style);

        Lampa.Listener.follow('line', e => { if (e.type === 'create' && e.data && e.line && e.line.render) { var el = e.line.render(); if (e.data.uas_title_row) el.attr('data-uas-title-row', 'true'); if (e.data.uas_content_row) el.attr('data-uas-content-row', 'true'); } });
        ...
        Lampa.Listener.follow('controller', e => { if (e.type === 'focus' && !initialFocusHandled) { initialFocusHandled = true; if ($(e.target).hasClass('card--title-btn')) setTimeout(() => Lampa.Controller.move('down'), 20); } });
        ...
        CardMaker.Card.onVisible = function () {
            ...
            var getContainer = posClass => {
                var clsName = 'badge-pos-' + posClass, container = view.querySelector('.card-custom-badges.' + clsName);
                if (!container) { container = document.createElement('div'); container.className = 'card-custom-badges ' + clsName; view.appendChild(container); }
                return container;
            };

            var badgePos = Lampa.Storage.get('uas_alt_badge_pos') || 'tl', ratingPos = Lampa.Storage.get('uas_alt_rating_pos') || 'tr';
            var targetContainer = useAltDesign ? getContainer(badgePos) : view, ratingContainer = getContainer(ratingPos);
            ...
            var doHeavyTasks = function() {
                if (Lampa.Storage.get('uas_ext_ratings_enable') === true && data.id && !isHistoryCard) {
                    ...
                    fetchExtRatings(data.id, data.media_type || (data.name ? 'tv' : 'movie')).then(ratings => {
                        if (!ratings || !cardInstance.html?.parentNode) return; 
                ...
                if (altDesignType === '1' && data.id && Lampa.Storage.get('uas_alt_design_apikey')) {
                    if (!isHistoryCard) {
                        getImdbIdForTmdb(data.id, data.media_type || (data.name ? 'tv' : 'movie')).then(async imdb => {
                            if (imdb) {
                                ...
                                if (isWideCard) {
                                    ...
                                    else { var t = new Image(); t.onload = () => AltImageCache.setAndGet(targetUrl).then(u => { view.style.backgroundImage = `url(${u})`; data.custom_full_bg = targetUrl; }); t.src = targetUrl; }
                                } else {
                                    ...
                                    if (img && !img.dataset.alt1Loaded) {
                                        ...
                                        if (cachedUrl) img.src = cachedUrl;
                                            ...
                                            var tmp = new Image();
                                            tmp.onload = () => AltImageCache.setAndGet(targetUrl).then(u => { img.src = u; img.classList.add('uas-poster-loaded'); });
                                            tmp.onerror = () => { img.src = fbSrc; }; tmp.src = targetUrl;
                                        }
                                    }
                                }
                            }
                ...
                if (Lampa.Storage.get('uas_show_flag') !== false && data.id) {
                    ...
                    if (!oldFlag) {
                        ...
                        if (meta) {
                            ...
                            else if (cached !== false) loadFlag(meta).then(isSuccess => { if (isSuccess && cardInstance.html?.parentNode) renderFlag(cardInstance.html, targetContainer, useAltDesign); });
                        }
                    } else {
                ...
                if (Lampa.Storage.get('uas_badge_season') !== false && (data.media_type === 'tv' || data.name || data.number_of_seasons) && data.id) {
                    fetchSeriesData(data.id).then(tmdbData => { if (cardInstance.html?.parentNode && cardInstance.data === data) renderSeasonBadge(cardInstance.html, tmdbData, targetContainer); }).catch(()=>{});
    ...
    if (window.appready) start(); else Lampa.Listener.follow('app', e => { if (e.type === 'ready') start(); });
