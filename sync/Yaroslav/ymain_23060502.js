(function () {
    ...
    function applyCardDesign(cardInstance, html, data) {
        ...
        var isHistoryCard = html.classList.contains('card--history-custom');
                ...
                emit: {
                    onCreate: function () {
                        ...
                        if (view.length) {
                            ...
                            var bgStyle = bgUrl ? `url(${bgUrl})` : 'rgba(30,30,30,0.8)';
                            view.css({ 
    ...
    function start() {
        ...
        style.innerHTML = `
            body:not(.uas-native-cards-mode) .card .card__age { display: none !important; }
            body.uas-native-cards-mode .card .card__age { display: none !important; }
        ...
        // =========================================================================
        // ПЕРЕХОПЛЕННЯ ФАБРИКИ КАРТОК LAMPA ДЛЯ МИТТЄВОГО ОНОВЛЕННЯ ДИЗАЙНУ
        // =========================================================================
        if (Lampa.Maker && Lampa.Maker.make) {
            var originalMakerMake = Lampa.Maker.make;
            Lampa.Maker.make = function (name, data, params) {
                var obj = originalMakerMake.apply(this, arguments);
                if (name === 'Card' && obj && obj.create) {
                    var originalCardCreate = obj.create;
                    obj.create = function () {
                        try {
                            originalCardCreate.apply(this, arguments);
                            if (obj.html && obj.data) {
                                var htmlEl = obj.html[0] || obj.html;
                                htmlEl.lampaCardInstance = obj;
                                
                                if (!obj.redrawBadgesAndAltDesign) {
                                    obj.redrawBadgesAndAltDesign = function() {
                                        applyCardDesign(obj, htmlEl, obj.data);
                                    };
                                }
                                obj.redrawBadgesAndAltDesign();
                            }
                        } catch (err) {}
                    };
                }
        ...
        var CardMaker = Lampa.Maker ? Lampa.Maker.map('Card') : null;
        if (CardMaker && CardMaker.Card) {
            var originalOnVisible = CardMaker.Card.onVisible;
            CardMaker.Card.onVisible = function () {
                // БЛОКУВАННЯ ЗАВАНТАЖЕННЯ ЗОБРАЖЕНЬ ЯКЩО АКТИВОВАНА ОПЦІЯ
                if (Lampa.Storage.get('uas_no_image_loading') === true) {
                ...
                var isSpecialCard = htmlEl.classList.contains('uas-custom-card') || htmlEl.classList.contains('card--title-btn') || htmlEl.classList.contains('card--collection-btn') || data.is_title_btn || data.is_collection_btn || data.is_placeholder;

                if (!isSpecialCard && typeof originalOnVisible === 'function') {
                ...
                if (!this.redrawBadgesAndAltDesign) {
                    this.redrawBadgesAndAltDesign = function() {
                        applyCardDesign(this, htmlEl, data);
                    };
                }
                this.redrawBadgesAndAltDesign();
            };
