(function() {
    ...
    // --- Менеджер модального вікна коментаря ---
    var CommentModal = {
        focusedCard: null,
        ...
        show: function(data, originalCardElement) {
                ...
                <div class="ua-comment-modal-overlay">
                    <div class="ua-comment-modal-window">
                        ...
                        <div class="ua-comment-meta" style="padding-right: 35px;">
                            <div class="ua-chip author">${sourceIcon}<span class="author-name">${data.author}</span>${reviewBadge}</div>
                        </div>
            ...
            var _this = this;
            setTimeout(function() {
                window.addEventListener('keydown', _this.closeHandler, true);
            }, 100);
        },
    ...
    // --- 3. Парсер коментарів (для інших сайтів) ---
    var parser = {
                ...
                network.req(searchUrl, function(html) {
                    ...
                    var checkLink = function() {
                        ...
                        network.req(targetUrl, function(page) {
                            ...
                            // ОНОВЛЕНА, ДУЖЕ ЖОРСТКА ПЕРЕВІРКА РОКУ (БЕЗ ВІДХИЛЕНЬ)
                            var isYearValid = false;
                            if (year) {
                                // 1. Шукаємо всі 4-значні роки в <title> сторінки
                                var titleYears = pageTitle.match(/\b(19\d{2}|20\d{2})\b/g);
                                
                                if (titleYears && titleYears.length > 0) {
                                    // Якщо в <title> є рік, він має ТОЧНО збігатися з нашим
                                    if (titleYears.indexOf(year.toString()) !== -1) {
                                        isYearValid = true;
                                    }
                                } else {
                                    // 2. Якщо в заголовку взагалі немає року, перевіряємо блоки з роком на самій сторінці.
                                    // Шукаємо ТІЛЬКИ поруч зі словом "рік/год/year" або у посиланнях вигляду /year/2002/
        ...
        this.fetch = function(movie) {
            ...
            var finish = function() {
                ...
                if (done >= totalSources) {
                    ...
                    // Усі рецензії завжди розміщуємо першими
