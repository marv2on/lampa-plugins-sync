    ...
    function InlineComments() {
        ...
        this.fetch = function(movie) {
            ...
            try {
                ...
                // --- UASerials (з додатковим дзеркалом uaserials.my та оригіналом uaserials.com) ---
                if (useUaSerials) {
                                                ...
                                                network.req(ajaxUrl, function(resJSON) {
                                                    try {
                                                        ...
                                                        try {
                                                            ...
                                                            if (json.comments !== undefined) htmlStr = json.comments;
                                                        } catch(e) {}
                                                        
                                                        if (htmlStr && htmlStr.trim().length > 10) {
                                                            var parsed = parseDleAjaxComments(htmlStr);
                                                            if (parsed.length > 0) {
                                                                fetchMainAjaxComments(cstart + 1, aggregated.concat(parsed));
                                                            } else {
                                                                data_us_main = aggregated;
                                                                usMainDone = true; checkUsComplete();
                                                            }
                                                        } else {
                                                            data_us_main = aggregated;
                                                ...
                                                network.req(ajaxUrl, function(resJSON) {
                                                    try {
                                                        ...
                                                        if (htmlStr && htmlStr.trim().length > 10) {
                                                            var parsed = parseDleAjaxComments(htmlStr);
                                                            if (parsed.length > 0) {
                                                                fetchAjaxComments(cstart + 1, aggregated.concat(parsed));
                                                            } else {
                                                                data_us_my = aggregated;
                                                                usMyDone = true; checkUsComplete();
                                                            }
                                                        } else {
                                                            data_us_my = aggregated;
