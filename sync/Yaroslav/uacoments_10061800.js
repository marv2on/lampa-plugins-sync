;(function() {
    ...
    var proxies =[ 'https://cors.lampa.stream/', 'https://cors.eu.org/' ];
    ...
    var network = {
        req: function(url, onSuccess, onError, proxyIdx) {
            ...
            $.ajax({
                url: getProxyUrl(proxies[pIdx], url), method: 'GET', dataType: 'text', timeout: 12000, 
                success: function(res) {
                    try {
                        ...
                        if (typeof body !== 'string' || body.length < 50) network.req(url, onSuccess, onError, pIdx + 1);
                        else onSuccess(body);
        ...
        this.fetch = function(movie) {
            ...
            try {
                ...
                if (useUaFlix) {
                    ...
                    var flixSearchTitle = movie.title || movie.name || '';
                    var expectedYearStrFlix = movie.release_date || movie.first_air_date || '';
                    var expectedYearMatchFlix = expectedYearStrFlix.match(/^(\d{4})/);
                    var expectedYearFlix = expectedYearMatchFlix ? parseInt(expectedYearMatchFlix[1], 10) : 0;

                    if (!flixSearchTitle) {
                        finishFlix([]);
                    } else {
                        // Точно так як потрібно для UAFlix
                        var searchUrlFlix = 'https://uafix.net/index.php?do=search&subaction=search&search_start=0&full_search=0&result_from=1&story=' + encodeURIComponent(flixSearchTitle);
                        
                        network.req(searchUrlFlix, function(html) {
                            if (flixCompleted) return;
                            var doc = parseHTML(html);
                            var links = [];
                            
                            // Збираємо можливі лінки на фільми з блоку контенту
                            var container = doc.find('#dle-content').length ? doc.find('#dle-content') : doc;
                            container.find('a[href]').each(function() {
                                var lnk = $(this).attr('href');
                                // Відсіюємо службові сторінки
                                if (lnk && lnk.length > 10 && 
                                    lnk.indexOf('/user/') === -1 && 
                                    lnk.indexOf('/page/') === -1 && 
                                    lnk.indexOf('do=search') === -1 &&
                                    lnk.indexOf('/tags/') === -1 &&
                                    lnk.indexOf('/xfsearch/') === -1 &&
                                    lnk.indexOf('#') === -1 &&
                                    lnk.indexOf('index.php') === -1) {
                                    
                                    var fullUrl = lnk.indexOf('http') === 0 ? lnk : 'https://uafix.net' + (lnk.indexOf('/') === 0 ? '' : '/') + lnk;
                                    
                                    // Фільтруємо категорійні лінки, залишаємо ті, що мають глибину
                                    var path = fullUrl.replace('https://uafix.net', '');
                            ...
                            links = links.slice(0, 8); // Перевіримо максимум 8 найперших варіантів

                            if (links.length === 0) return finishFlix([]);

                            var lIdx = 0;
                            var checkFlixLink = function() {
                                if (lIdx >= links.length) return finishFlix([]);
                                ...
                                network.req(targetUrl, function(pageHtml) {
                                    ...
                                    var pageTitle = '';
                                    var headMatch = pageHtml.match(/<title[^>]*>([\s\S]*?)<\/title>/i);
                                    if (headMatch && headMatch[1]) pageTitle = headMatch[1];
                                    else {
                                        var pDoc = parseHTML(pageHtml);
                                        pageTitle = pDoc.find('title').text() || '';
                                    }

                                    // Розбиваємо Title
                                    var parsedTitle = pageTitle;
                                    var parsedInside = "";
                                    var titleRegex = /^(.*?)\s*\(([^)]+)\)/;
                                    var match = pageTitle.match(titleRegex);
                                    if (match) {
                                        parsedTitle = match[1].trim();
                                        parsedInside = match[2].trim();
                                    }

                                    // Шукаємо рік
                                    var yearMatch = (parsedInside || pageTitle).match(/\b(19\d{2}|20\d{2})\b/);
                                    var pageYear = yearMatch ? parseInt(yearMatch[1], 10) : 0;
                                    var matchFound = false;

                                    // Якщо рік у дужках є, і він не збігається — відхиляємо
                                    if (expectedYearFlix > 0 && pageYear > 0 && pageYear !== expectedYearFlix) {
                                        matchFound = false;
                                    } else {
                                        // Очищуємо сміття з назв для точного порівняння
                                        var getClean = function(str) {
                                        ...
                                        var cleanSearch = getClean(flixSearchTitle);
                                        var cleanPage = getClean(parsedTitle);

                                        var isSimilar = false;
                                        if (cleanSearch && cleanPage) {
                                            if (cleanPage.indexOf(cleanSearch) !== -1 || cleanSearch.indexOf(cleanPage) !== -1) {
                                                isSimilar = true;
                                            } else {
                                                // Звіряємо слова, достатньо 30% збігу
                                                var sWords = cleanSearch.split(' ');
                                                var pWords = cleanPage.split(' ');
                                                var matches = 0;
                                                for (var w = 0; w < sWords.length; w++) {
                                                    if (sWords[w].length > 1 && pWords.indexOf(sWords[w]) !== -1) matches++;
                                                }
                                                if (sWords.length > 0 && (matches / sWords.length >= 0.30)) {
                                                    isSimilar = true;
                                                } else {
                                                    var minLen = Math.min(cleanSearch.length, cleanPage.length);
                                                    var maxLen = Math.max(cleanSearch.length, cleanPage.length);
                                                    if (maxLen > 0 && (minLen / maxLen >= 0.30)) isSimilar = true;
                                                }
                                            }
                                        } else if (!cleanSearch) {
                                            // Якщо після очищення оригінальна назва порожня (напр. була просто "Серіал")
                                            isSimilar = true;
                                        }

                                        if (isSimilar) matchFound = true;
                                    }

                                    if (matchFound) {
                                        finishFlix(parser.parse(pageHtml, 'UAFlix', '#dle-comments-list, .comments'));
                                    } else {
                                        checkFlixLink(); // Пробуємо наступний лінк
                                    }
                                }, function() { checkFlixLink(); });
                            };
                            checkFlixLink();
                        }, function() { finishFlix([]); });
                    }
                ...
                if (useKinoBaza) {
                    ...
                    if (!kbSearchTitle) {
                        ...
                        var expectedYear = yearMatch ? parseInt(yearMatch[1]) : 0;
                        ...
                        network.req(kbSearchUrl, function(responseStr) {
                            ...
                            try {
                                ...
                                var calculateSimilarity = function(str1, str2) {
                                    var clean1 = str1.toLowerCase().replace(/[^a-z0-9а-яіїєґ]/gi, '');
                                    var clean2 = str2.toLowerCase().replace(/[^a-z0-9а-яіїєґ]/gi, '');
                                ...
                                for (var i = 0; i < items.length; i++) {
                                    ...
                                    var itemYearMatch = itemYearStr.match(/^(\d{4})/);
                                    var itemYear = itemYearMatch ? parseInt(itemYearMatch[1]) : 0;
                                    
                                    if (expectedYear && itemYear && expectedYear !== itemYear) continue;

                                    var nameEn = item.name_en || item.name || item.name_ua || '';
                                    if (!nameEn) continue;

                                    var simScore = calculateSimilarity(kbSearchTitle, nameEn);
                                    var isSimilar = finder.areTitlesSimilar(kbSearchTitle, nameEn);
                                    
                                    if (simScore >= 0.9 || isSimilar) {
                                        if (item.slug) {
                                            var slug = item.slug;
