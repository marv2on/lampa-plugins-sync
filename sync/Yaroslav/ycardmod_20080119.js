	...
	function fetchNetflixTrailer(tmdb_id, media_type, callback) {
        ...
        $.ajax({
            ...
            success: function(title_data) {
                if (title_data && title_data.netflix_id) {
                    var preview_url = server + "/api/preview/" + title_data.netflix_id + '?' + qs;
                    $.ajax({
                        url: preview_url,
                        type: 'GET',
                        dataType: 'json',
                        success: function(preview_data) {
                            var clips = preview_data && preview_data.clips || [];
                            var trailers = clips.filter(function(c) { return c.kind === 'trailer'; });
                            var list = trailers.length ? trailers : clips;
                            if (list.length && list[0].url) {
                                callback(list[0].url);
                            } else {
                                callback(null);
                            }
			...
			if (Type.co(e)) {
                ...
                if (isRunTrailers || isBgTrailers) {
                    ...
                    if (trailer_source === 'all') {
                        sources_list = ['imdb', 'tmdb', 'netflix'];
                    } else {
