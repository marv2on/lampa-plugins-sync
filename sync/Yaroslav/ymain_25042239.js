(function () {
    ...
    function renderFlag(cardHtml, targetContainer, useAltDesign) {
        ...
        var leftContainer = view.querySelector('.card-left-badges');
        if (leftContainer) {
    ...
    function fetchLogo(movie, itemElement) {
        ...
        if (useAltDesign && isWideCard) {
            // В Альт-режимі на головній завантажуємо ЛИШЕ чистий бекдроп. Жодних логотипів.
    ...
    async function fetchPageData(targetPage, baseUrl, isLME, isKinobazaOnline, isUasCollection, isUasCollectionsList, params) {
        ...
        if (isLME) {
            ...
            if (listCache[listUrl]) {
                ...
                let items = extractUaserialsCollections(html); // Виправлено назву функції
    ...
    function start() {
        ...
        style.innerHTML = `
            ...
            /* Стандартний дизайн вертикальних карток */
            ...
            /* --- АЛЬТЕРНАТИВНИЙ ДИЗАЙН (ГЛОБАЛЬНО ДЛЯ ВСІХ КАРТОК) --- */
            
            /* 1. Повне приховування рейтингу на ВСІХ картках */
            body.uas-alt-design-active .card__vote,
            body.uas-alt-design-active .card-rating,
            body.uas-alt-design-active .card .card__vote { 
                display: none !important; opacity: 0 !important; visibility: hidden !important; 
            }
            
            /* 2. Зняття затемнень на широких бекдропах (головна) */
            body.uas-alt-design-active .card--wide-custom .card-backdrop-overlay {
                display: none !important; background: transparent !important;
            }
            body.uas-alt-design-active .card--wide-custom .card__view::after,
            body.uas-alt-design-active .card--wide-custom .card__view::before {
                display: none !important; content: none !important;
            }

            /* 3. Приховування всіх логотипів на головній сторінці (широкі картки) */
            body.uas-alt-design-active .card--wide-custom .card-custom-logo,
            body.uas-alt-design-active .card--wide-custom .card-custom-logo-text {
                display: none !important; visibility: hidden !important; opacity: 0 !important;
            }
            
            /* 4. Контейнер для бейджів зліва (Вертикальні та Широкі картки) */
            .card.uas-alt-card .card-left-badges {
            ...
            /* 5. ІДЕАЛЬНІ ОВАЛЬНІ БЕЙДЖІ (Таблетки, як на референсі) */
            .card.uas-alt-card .card-left-badges > div {
                position: static !important; /* Очищуємо абсолютне позиціювання Lampa */
                margin: 0 !important; 
                height: 1.8em !important; /* Збільшена висота для простору над/під текстом */
                min-height: 1.8em !important;
                width: auto !important;
                border-radius: 2em !important; /* Ідеальний овал з 4 сторін */
                display: flex !important; 
                align-items: center !important; 
                justify-content: center !important;
                background: rgba(0, 0, 0, 0.55) !important; /* 60% прозорості чорного */
                padding: 0 0.6em !important; /* Простір навколо тексту з боків */
                font-size: 0.7em !important; /* Уніфікований розмір шрифту */
                font-weight: 700 !important;
                color: #fff !important; /* Завжди білий шрифт */
                box-shadow: none !important; /* Тінь прибрано */
                line-height: 1 !important;
                border: 1px solid rgba(255,255,255,0.15) !important; /* Легка рамка */
                box-sizing: border-box !important;
            }
            
            /* Порядок виведення бейджів */
            .card.uas-alt-card .card-left-badges .card-badge-age { order: 1 !important; }
            .card.uas-alt-card .card-left-badges .card__type { order: 2 !important; }
            
            /* 6. Текстовий бейдж UA (замість прапора) */
            .card.uas-alt-card .card-left-badges .card__ua_flag { 
                order: 3 !important; 
                background: rgba(0, 0, 0, 0.55) !important; /* 60% прозорості чорного */
                ...
                text-shadow: none !important; /* Тінь тексту прибрано */
                box-shadow: none !important; /* Тінь блоку прибрано */
            }

            /* --- ГЛОБАЛЬНІ НАЛАШТУВАННЯ ГОЛОВНОЇ СТОРІНКИ --- */
            
            /* Приховування прапорців та UA на головній сторінці (для всіх стилів) */
            body.uas-main-active .card__ua_flag {
                display: none !important; 
                visibility: hidden !important; 
                opacity: 0 !important;
            }

            /* 7. ПРИХОВУВАННЯ ВСІХ БЕЙДЖІВ НА КАРТКАХ ІСТОРІЇ (Для всіх стилів) */
            ...
            .card--history-custom .card__ua_flag {
                display: none !important; 
                visibility: hidden !important; 
                opacity: 0 !important;
            }
        ...
        Lampa.Listener.follow('activity', function (e) {
            if (e.type === 'start') {
                ...
                // Встановлення класу Альт Дизайну глобально
                if (Lampa.Storage.get('uas_alt_design_enable') === true) {
        ...
        CardMaker.Card.onVisible = function () {
            ...
            // Контейнер для овальних бейджів зліва ВСЮДИ (включаючи широкі) при Альт-дизайні
            // (Крім карток історії, для них це відключено в CSS)
            ...
            // Жорстке приховування стандартного рейтингу TMDB через JS
            ...
            // Альт Дизайн - Постери та Бекдропи виключно через прямі лінки
                ...
                getImdbIdForTmdb(data.id, data.media_type || (data.name ? 'tv' : 'movie')).then(function(imdb) {
                    if (imdb) {
                        if (isWideCard || isHistoryCard) {
                            ...
                            if (img) {
                                // Клонування розриває зв'язок з Lampa, щоб вона не переписала постер
            ...
            // Бейдж ПРАПОРЦЯ (При альт-дизайні — текст "UA", інакше — градієнтний бейдж)
