(function () {
    ...
    async function fetchHtml(url) {
        ...
        for (let proxy of PROXIES) {
            try {
                ...
                let res = await fetch(fetchUrl, { signal: controller.signal });
                if (res.ok) {
    ...
    // Спеціальний метод для отримання HTML за допомогою POST-запиту
    async function fetchHtmlPost(url, paramsObj) {
        let controller = new AbortController();
        let timeoutId = setTimeout(() => controller.abort(), 12000);
        ...
        for (let proxy of PROXIES) {
            // allorigins не підтримує POST у такому форматі, тому пропускаємо
            if (proxy.includes('allorigins')) continue;
            ...
            try {
                ...
                if (res.ok) {
                    let text = await res.text();
                    // DLE AJAX відповідь може бути коротшою за звичайну сторінку, тому перевірка довжини менша (напр. > 50)
    ...
    function extractUaFlixLinks(html) {
        ...
        container.querySelectorAll('a[href]').forEach(a => {
            let href = a.getAttribute('href');
            if (href && (href.includes('/films/') || href.includes('/serials/') || href.includes('/cartoons/'))) {
                let path = href.replace(/^https?:\/\/[^\/]+/, '').split('?')[0].split('#')[0];
                let parts = path.split('/').filter(Boolean);
                
                if (parts.length >= 2 && !path.includes('page/')) {
                    let isNav = a.closest('nav, header, footer, .sidebar, .menu, .sort, .navigation, .sect-header, .sect-content-top');
                    if (!isNav) {
    ...
    function extractUaFlixDetails(itemHtml, link) {
        ...
        let origTitle = '', year = '';
        let type = link.includes('/serials/') ? 'tv' : link.includes('/films/') ? 'movie' : null;
        ...
        if (engRus && engRus.textContent.trim()) {
            ...
            for (let i = 0; i < lis.length; i++) {
                ...
                if (tc.includes('Ориг. назва:') || tc.includes('Оригінальна назва:')) {
        ...
        if (yearSpan && yearSpan.textContent.trim()) {
            ...
            let lis = doc.querySelectorAll('ul.finfo li');
            for (let i = 0; i < lis.length; i++) {
                if (lis[i].textContent.includes('Рік виходу:')) {
                    let clone = lis[i].cloneNode(true);
                    let s = clone.querySelector('span');
                    if (s) s.remove();
    ...
    async function fetchUaFlixCatalog(url, limit, noCache) {
        ...
        // Використовуємо POST для сортування за датою (від нових)
        let html = await fetchHtmlPost(url, {
            'xf_sort': 'get',
            'xf_field': 'default',
            'xf_value': 'date'
        });
        
        // Якщо POST не повернув достатньо HTML, резервно використовуємо GET
        if (!html || html.length < 100) {
            html = await fetchHtml(url);
        }

        let links = extractUaFlixLinks(html);
        ...
        links = links.filter(link => {
            let p = link.split('/').filter(Boolean);
            let lastPart = p[p.length - 1] || '';
            return !excludeWords.includes(lastPart.toLowerCase());
        });
        ...
        await new Promise(resolve => {
            ...
            function next() {
                ...
                while (active < concurrency && index < links.length && currentValidCount < limit) {
                    ...
                    (async () => {
                        try {
                            let itemHtml = await fetchHtml(link);
                            if (itemHtml) {
                                let details = extractUaFlixDetails(itemHtml, link);
                                if (details && details.title && details.year) {
                                    ...
                                    if (tmdbRes && tmdbRes.id && (tmdbRes.backdrop_path || tmdbRes.poster_path) && !unique[tmdbRes.id]) {
                                        ...
                                        resultsArray[currentIndex] = tmdbRes;
                                    }
                                }
                            }
                        } catch(e) {}
    ...
    async function searchTmdbByTitleAndYear(title, year, expectedType) {
        ...
        for (let path of endpoints) {
            try {
                let dataUk = await fetch(getTmdbEndpoint(`${path}?query=${encodeURIComponent(title)}&language=uk`)).then(r => r.json());
                let dataEn = await fetch(getTmdbEndpoint(`${path}?query=${encodeURIComponent(title)}&language=en`)).then(r => r.json());
                
                let combined = [...(dataUk.results || []), ...(dataEn.results || [])];
                ...
                if (results.length) {
                    for (let r of results) {
                        ...
                        if (yearDiff <= 1 || !year) {
                            ...
                            // Точне співпадіння отримує найбільший бал
                            ...
                            else if (t1.includes(s) || t2.includes(s) || s.includes(t1) || s.includes(t2)) score += 5;
            ...
            // Якщо знайдено ідеальний збіг (точний рік і точна назва = 80 балів), завершуємо пошук
        ...
        if (bestResult) {
            ...
            if (!res.overview || !res.overview.trim()) {
                try {
                    ...
                    let enData = await fetch(getTmdbEndpoint(`${enType}/${res.id}?language=en`)).then(r => r.json());
            ...
            params: {
                ...
                emit: {
                    onCreate: function () {
                        ...
                        var imgUrl = imgPath ? Lampa.TMDB.image('t/p/' + quality + imgPath) : '';
    ...
    function start() {
        ...
        // Оновлення порядку за замовчуванням
        if (!Lampa.Storage.get('ym_rows_init_v8_fix_11')) {
            Lampa.Storage.set('ym_rows_init_v8_fix_11', true);
            DEFAULT_ROWS_SETTINGS.forEach(function(r) { 
