(function () {
    ...
    function getActiveProxies() {
        var custom = Lampa.Storage.get('uas_custom_proxies');
        var arr = [];
        if (custom) {
            arr = custom.split(',').map(function(p) { return p.trim(); }).filter(function(p) { return p; });
        }
        return arr.concat(PROXIES);
    }

    const DEFAULT_ROWS_SETTINGS =[
        { id: 'ym_row_history', title: 'Історія перегляду', defOrder: '1', default: true },
        { id: 'ym_row_uaflix_films', title: 'Uaflix фільми', defOrder: '2', default: true },
        { id: 'ym_row_uaflix_series', title: 'Uaflix серіали', defOrder: '3', default: true },
        { id: 'ym_row_movies_new', title: 'Новинки фільмів', defOrder: '4', default: true },
        { id: 'ym_row_series_new', title: 'Новинки серіалів', defOrder: '5', default: true },
        { id: 'ym_row_collections', title: 'Підбірки', defOrder: '6', default: true },
        { id: 'ym_row_kinobaza', title: 'Новинки Стрімінгів UA', defOrder: '7', default: true },
        { id: 'ym_row_community', title: 'Приховані геми LME', defOrder: '8', default: true },
        { id: 'ym_row_movies_watch', title: 'Популярні фільми', defOrder: '9', default: true },
        { id: 'ym_row_series_pop', title: 'Популярні серіали', defOrder: '10', default: true },
        { id: 'ym_row_random', title: 'Випадкові фільми', defOrder: '11', default: true }
    ];
    ...
    var rateIcons = {
        imdb: "data:image/svg+xml;charset=utf-8,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 448 512'%3E%3Cpath fill='%23f5c518' d='M400 32H48C21.5 32 0 53.5 0 80v352c0 26.5 21.5 48 48 48h352c26.5 0 48-21.5 48-48V80c0-26.5-21.5-48-48-48z'/%3E%3Cpath d='M125.7 344H101V169.5h24.7V344zm76.5 0h-23.7V229l-16.7 115h-21l-16.8-115V344H101V169.5h27.4l20.4 122 20.2-122H196L202.2 344zm75.6 0h-35.3c-15.6 0-24.8-11.2-24.8-29.2V198.8c0-18 9.2-29.2 24.8-29.2h35.3c21 0 32.7 14.8 32.7 41.5v91.3c0 26.8-11.7 41.6-32.7 41.6zm-10.4-154.5h-10.7v134.5h10.7c7.4 0 10.3-6.2 10.3-21v-92.4c0-14.8-2.9-21.1-10.3-21.1zm98.1 154.5h-26V169.5h33.8c18.5 0 28.5 8.7 28.5 26.7v37.6c0 14.5-5.5 22.3-14.3 25.5 10 2.8 16.5 11 16.5 26.8v41c0 10.6-2 16.9-18.5 16.9zm-1.1-152h-9v49h9c5.2 0 7.3-3 7.3-11v-27c0-8-2-11-7.3-11zm1.2 73h-10v58h10c5.5 0 7.8-3.4 7.8-12v-34c0-8.6-2.3-12-7.8-12z'/%3E%3C/svg%3E",
        tmdb: "data:image/svg+xml;charset=utf-8,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 512 512'%3E%3Crect width='512' height='512' rx='100' fill='%230d253f'/%3E%3Cpath fill='%2301b4e4' d='M256 120a136 136 0 100 272 136 136 0 000-272z'/%3E%3C/svg%3E", 
        rt: "data:image/svg+xml;charset=utf-8,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 512 512'%3E%3Ccircle cx='256' cy='256' r='200' fill='%23fa320a'/%3E%3C/svg%3E", 
        mc: "data:image/svg+xml;charset=utf-8,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 512 512'%3E%3Ccircle cx='256' cy='256' r='200' fill='%23ffcc33'/%3E%3Cpath d='M180 200h150v112H180z' fill='%23fff'/%3E%3C/svg%3E",
        trakt: "data:image/svg+xml;charset=utf-8,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 512 512'%3E%3Cpath fill='%23ed1c24' d='M256 32L32 144v224l224 112 224-112V144z'/%3E%3C/svg%3E",
        popcorn: "data:image/svg+xml;charset=utf-8,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 512 512'%3E%3Cpath fill='%23f5b50a' d='M100 200h312v250H100z'/%3E%3C/svg%3E",
        letterboxd: "data:image/svg+xml;charset=utf-8,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 512 512'%3E%3Ccircle cx='156' cy='256' r='60' fill='%23209ce4'/%3E%3Ccircle cx='256' cy='256' r='60' fill='%2300e054'/%3E%3Ccircle cx='356' cy='256' r='60' fill='%23ff8000'/%3E%3C/svg%3E"
    };
    ...
    function getCardMapFn() {
        if (window.YDesignLoaded) {
            return makeStandardCardItem;
        }
        var nativeMode = Lampa.Storage.get('uas_native_cards_mode') === true;
        return (nativeMode || isMobile()) ? makeStandardCardItem : makeWideCardItem;
    }

    function normalizeMovieData(movie, forceLoadImages) {
        if (movie) {
            if (movie.is_title_btn || movie.is_collection_btn) return movie;
            var yr = String(movie.release_date || movie.first_air_date || movie.year || '').substring(0, 4).trim();
            movie.release_year = yr || ' '; 
            movie.year = yr || ' ';
            if (Lampa.Storage.get('uas_no_image_loading') !== true || forceLoadImages) {
                if (!movie.img && movie.poster_path) {
                    var quality = Lampa.Storage.get('ym_img_quality') || 'w300';
                    movie.img = Lampa.TMDB.image('t/p/' + quality + movie.poster_path);
                }
            }
        }
        return movie;
    ...
    function getAltDesignType() {
        if (Lampa.Storage.get('uas_native_cards_mode') === true) return '0';
    ...
    function updateDynamicStyles() {
        var nativeMode = Lampa.Storage.get('uas_native_cards_mode') === true;
        if (nativeMode) document.body.classList.add('uas-native-cards-mode');
        else document.body.classList.remove('uas-native-cards-mode');

        var altType = getAltDesignType();
        if (altType === '1' || altType === '2') document.body.classList.add('uas-alt-design-active');
        else document.body.classList.remove('uas-alt-design-active');
        
        if (altType === '2') document.body.classList.add('uas-alt-design-2');
        else document.body.classList.remove('uas-alt-design-2');
        ...
        document.body.style.setProperty('--ym-row-title-size', (Lampa.Storage.get('ym_row_title_size') || '1.4') + 'em');

        var bgMode = getBgMode();
        if (bgMode === '0') document.body.classList.add('uas-bg-disabled');
        else document.body.classList.remove('uas-bg-disabled');
        if (bgMode === '2') document.body.classList.add('uas-bg-instant');
        else document.body.classList.remove('uas-bg-instant');
    }
    ...
    var BgManager = {
        ...
        change: function(url, instant) {
            ...
            if (instant) execute(); else this.timer = setTimeout(execute, 0);
        },
    ...
    // === ФУНКЦІЯ СИНХРОННОГО ЗАСТОСУВАННЯ ДИЗАЙНУ ДО КАРТОК ===
    function applyCardDesign(cardInstance, html, data) {
        if (!html || !data) return;

        var isHistoryCard = html.classList.contains('card--history-custom') || data.is_history_card;
        var isSpecialCard = html.classList.contains('uas-custom-card') || html.classList.contains('card--title-btn') || html.classList.contains('card--collection-btn') || data.is_title_btn || data.is_collection_btn || data.is_placeholder;
        var noImageLoading = Lampa.Storage.get('uas_no_image_loading') === true;
        
        // Повністю запобігаємо втручанню ydes.js в історію та спец-кнопки
        if (isHistoryCard || isSpecialCard) {
            html._ydesign_built = true; 
        }

        // Передаємо управління дизайном для звичайних карток на ydes.js
        if (window.YDesignLoaded && !isHistoryCard && !isSpecialCard) {
            return;
        }

        var altDesignType = getAltDesignType(), useAltDesign = altDesignType === '1' || altDesignType === '2';
        var isWideCard = html.classList.contains('card--wide-custom');
        
        var nativeMode = Lampa.Storage.get('uas_native_cards_mode') === true;
        
        var isStandardVerticalAlt2 = altDesignType === '2' && !isWideCard && !isHistoryCard && !isSpecialCard && Lampa.Storage.get('uas_alt2_standard_poster') !== true && !noImageLoading;

        if (isStandardVerticalAlt2 && !nativeMode) {
        ...
        if (isSpecialCard || isHistoryCard) return;
        ...
        if (hasYearBadge) html.classList.add('uas-hide-original-age');
        else html.classList.remove('uas-hide-original-age');

        // 2. ЗОВНІШНІ РЕЙТИНГИ
        if (Lampa.Storage.get('uas_ext_ratings_enable') === true && data.id) {
            ...
            if (Lampa.Storage.get('uas_ext_rt_tmdb') !== false && data.vote_average > 0) {
                if (!ratingContainer.querySelector('.ext-rt-tmdb')) {
                    let badge = document.createElement('div'); badge.className = 'ext-rt-tmdb ext-rating-badge'; badge.innerHTML = `<img src="${rateIcons.tmdb}" onload="this.style.opacity=1" style="opacity:0"><span>${data.vote_average.toFixed(1)}</span>`; ratingContainer.appendChild(badge);
                }
            }
            fetchExtRatings(data.id, data.media_type || (data.name ? 'tv' : 'movie')).then(function(ratings) {
                if (!ratings || !html) return; 
                ['imdb', 'rt', 'mc', 'letterboxd', 'trakt', 'mdblist', 'popcorn'].forEach(function(key) {
                    if (ratings[key] && Lampa.Storage.get('uas_ext_rt_' + key) !== false && !ratingContainer.querySelector('.ext-rt-' + key)) {
                        ...
                        if (displayVal) { badge.className = 'ext-rt-' + key + ' ext-rating-badge'; badge.innerHTML = `<img src="${rateIcons[key]}" onload="this.style.opacity=1" style="opacity:0"><span>${displayVal}</span>`; ratingContainer.appendChild(badge); }
                    }
                });
            });
        } else if (altDesignType === '2' && data.vote_average && showRating) {
            var voteValAlt = parseFloat(data.vote_average);
            if (!isNaN(voteValAlt) && voteValAlt > 0) {
                var rBadge = view.querySelector('.card-alt-rating');
                if (!rBadge) { rBadge = document.createElement('div'); rBadge.className = 'card-alt-rating ext-rating-badge'; rBadge.innerHTML = `<img src="${rateIcons.tmdb}" onload="this.style.opacity=1" style="opacity:0"><span>${voteValAlt.toFixed(1)}</span>`; ratingContainer.appendChild(rBadge); }
                else if (rBadge.parentNode !== ratingContainer) ratingContainer.appendChild(rBadge);
            }
        }

        // 3. УКРАЇНСЬКИЙ ПРАПОРЕЦЬ
        if (Lampa.Storage.get('uas_show_flag') !== false && data.id) {
            ...
            if (!oldFlag) {
                ...
                if (meta) {
                    ...
                    else if (cached !== false) loadFlag(meta).then(function(isSuccess) { if (isSuccess && html) renderFlag(html, targetContainer, useAltDesign); });
                }
            } else {
        ...
        // 4. БЕЙДЖ СЕЗОНУ / СЕРІЇ
        if (Lampa.Storage.get('uas_badge_season') !== false && (data.media_type === 'tv' || data.name || data.number_of_seasons) && data.id) {
            fetchSeriesData(data.id).then(function(tmdbData) { if (html) renderSeasonBadge(html, tmdbData, targetContainer); }).catch(function(){});
        ...
        // 5. ВАЖКІ ЗАВДАННЯ: завантаження лого та альтернативних зображень (миттєве)
        var loadAltImagesAndLogos = function() {
            if (noImageLoading) return;
                    ...
                    if (imdb) {
                        ...
                        if (isWideCard) {
                            ...
                            if (img && !img.dataset.alt1Loaded) {
                                ...
                                var quality = Lampa.Storage.get('ym_img_quality') || 'w300';
                                var fbSrc = Lampa.TMDB.image('t/p/' + quality + data.poster_path);
        ...
        // Завантажуємо зображення та логотипи миттєво, щоб уникнути сірих карток
        loadAltImagesAndLogos();
    }

    // --- SMART TMDB FETCH WITH ABORT CONTROLLER & PROXY FALLBACK ---
    async function advancedTmdbFetch(url) {
        try {
            ...
            let timeoutId = setTimeout(() => { if(controller) controller.abort(); }, 1500); 
            ...
            // Ignored, fallback to proxies below
        }

        let proxyList = getActiveProxies();
        for (let i = 0; i < proxyList.length; i++) {
            try {
                let proxy = proxyList[i];
                let fetchUrl = proxy.indexOf('?url=') !== -1 ? proxy + encodeURIComponent(url) : proxy + url;
                
                let pController = typeof AbortController !== 'undefined' ? new AbortController() : null;
                let pSignal = pController ? pController.signal : undefined;
                let pTimeoutId = setTimeout(() => { if(pController) pController.abort(); }, 4000); 
                
                let res = await fetch(fetchUrl, { signal: pSignal });
    ...
    async function getImdbIdForTmdb(tmdbId, type) {
        ...
        try {
            let res = await advancedTmdbFetch(getTmdbEndpoint(`${type}/${tmdbId}/external_ids`));
    ...
    async function fetchTmdbWithFallback(type, id) {
        let res = await advancedTmdbFetch(getTmdbEndpoint(`${type}/${id}?language=uk`));
        if (res && (!res.overview || !res.overview.trim())) {
            let enRes = await advancedTmdbFetch(getTmdbEndpoint(`${type}/${id}?language=en`));
    ...
    function loadFlag(meta) {
        if (!inflight[meta.cacheKey]) {
            inflight[meta.cacheKey] = new Promise(resolve => {
                ...
                Lampa.Network.silent(url, r => resolve(isSuccessResponse(r)), () => resolve(false), null, { timeout: CONFIG.timeout });
            }).then(isSuccess => { 
    ...
    function fetchSeriesData(tmdbId) {
        return new Promise((resolve, reject) => {
            ...
            advancedTmdbFetch(getTmdbEndpoint(`tv/${tmdbId}?language=${CONFIG.language}`)).then(data => {
                if (data) {
                    ...
                    resolve(data);
                } else reject(new Error('fetchSeriesData returned null'));
            }).catch(reject);
        });
    }
    ...
    function extractUaFlixLinks(html, typeFilter) {
        let doc = new DOMParser().parseFromString(html, "text/html"), links = [], seen = {};
        let container = doc.querySelector('#dle-content') || doc.querySelector('.main') || doc.body;
        if (!container) container = doc;

        let els = container.querySelectorAll('a[href]');
        // Великий список виключень для ігнорування категорій/жанрів
        let excludeWords = ['action', 'comedy', 'drama', 'thriller', 'horror', 'detective', 'melodrama', 'fantasy', 'adventure', 'family', 'anime', 'documentary', 'biography', 'history', 'sport', 'music', 'western', 'war', 'ukrainian', 'dubbing', 'new_netflix_film_ua', 'new_uaserial', 'collections', 'multfilmy', 'tv_shou', 'komediyi', 'boyovyky', 'detektyvy', 'dramy', 'zhahy', 'melodramy', 'prygody', 'simeyni', 'fentezi', 'fantastyka', 'istorychni', 'mestyka', 'mistyka', 'detektiv', 'komedija', 'tryler', 'zhasy'];

        for (let i = 0; i < els.length; i++) {
            let a = els[i];
            let href = a.getAttribute('href');
            if (href && href.indexOf('/' + typeFilter + '/') !== -1) {
                let path = href.replace(/^https?:\/\/[^\/]+/, '').split('?')[0].split('#')[0];
                let parts = path.split('/').filter(Boolean);
                
                // Переконуємось що це шлях типу /films/title/
                if (parts.length >= 2 && path.indexOf('page/') === -1) {
                    let isNav = false;
                    if (a.closest) {
                        isNav = a.closest('nav, header, footer, .sidebar, .menu, .sort, .navigation, .sect-header, .sect-content-top, .pagination');
                    }
                    if (!isNav) {
                        // Відкидаємо можливе .html якщо раптом є
    ...
    function extractUaFlixDetails(itemHtml, link) {
        ...
        if (engRus && engRus.textContent.trim()) {
            ...
            let lis = doc.querySelectorAll('ul.finfo li, ul.f-info li, .finfo li, .full-info li');
            for (let i = 0; i < lis.length; i++) {
                let tc = lis[i].textContent;
                if (tc.indexOf('Ориг. назва:') !== -1 || tc.indexOf('Оригінальна назва:') !== -1 || tc.indexOf('Оригинальное название:') !== -1) {
        ...
        if (!origTitle) {
            let h1 = doc.querySelector('h1[itemprop="name"], h1.title, h1');
        ...
        if (yearSpan && yearSpan.textContent.trim()) {
            ...
            let lis = doc.querySelectorAll('ul.finfo li, ul.f-info li, .finfo li, .full-info li');
            for (let i = 0; i < lis.length; i++) {
                if (lis[i].textContent.indexOf('Рік виходу:') !== -1 || lis[i].textContent.indexOf('Рік:') !== -1 || lis[i].textContent.indexOf('Год:') !== -1) {
                    let clone = lis[i].cloneNode(true);
                    let s = clone.querySelector('span');
                    if (s) s.remove();
        ...
        if (!year) {
            let h1 = doc.querySelector('h1[itemprop="name"], h1.title, h1');
            if (h1 && h1.textContent.trim()) {
                let match = h1.textContent.match(/\((\d{4})\)/);
        ...
        if (type === 'tv') {
            origTitle = origTitle.replace(/\[.*?\]/g, '').replace(/\(.*\)/g, '').trim();
        }
    ...
    async function fetchUaFlixCatalog(url, limit, noCache, typeFilter) {
        if (!noCache && listCache[url + '_' + typeFilter]) return listCache[url + '_' + typeFilter];
        
        let html = await fetchUaFlixHtml(url, 4000);

        let links = extractUaFlixLinks(html, typeFilter);
        
        // Для головної сторінки беремо лише необхідну частину лінків щоб не навантажувати систему
        // В каталозі перевіряються всі доступні лінки на сторінці (але не відкидається жодна картка)
        if (url === 'https://uafix.net/') {
            links = links.slice(0, limit + 10);
        }

        let promises = links.map(async (link) => {
            try {
                let details = uaFlixDetailsCache[link];
                if (!details) {
                    let itemHtml = await fetchUaFlixHtml(link, 2500); 
                    if (itemHtml) {
                        details = extractUaFlixDetails(itemHtml, link);
                        if (details && details.title && details.year) {
                            uaFlixDetailsCache[link] = details;
                        }
                    }
                }
                if (details && details.title && details.year) {
                    let tmdbRes = await searchTmdbByTitleAndYear(details.title, details.year, details.type);
                    if (tmdbRes && tmdbRes.id && (tmdbRes.backdrop_path || tmdbRes.poster_path)) {
                        return tmdbRes;
                    }
                }
            } catch(e) {}
        ...
        if (!noCache && finalItems.length) listCache[url + '_' + typeFilter] = finalItems; 
        return finalItems;
    }
    ...
    async function processInQueue(items, processFn) {
        let promises = items.map(async item => {
            try { return await processFn(item); } catch (e) { return null; }
        });
    ...
    async function processSingleItem(url) {
        ...
        try {
            let data = await advancedTmdbFetch(getTmdbEndpoint(`find/${imdb}?external_source=imdb_id&language=uk`));
            if (!data) return null;
            ...
            if (res && (!res.overview || !res.overview.trim())) {
                let enData = await advancedTmdbFetch(getTmdbEndpoint(`find/${imdb}?external_source=imdb_id&language=en`));
                if (enData) {
    ...
    async function searchTmdbByTitleAndYear(title, year, expectedType) {
        ...
        for (let pIdx = 0; pIdx < endpoints.length; pIdx++) {
            ...
            try {
                let dataUk = await advancedTmdbFetch(getTmdbEndpoint(`${path}?query=${encodeURIComponent(title)}&language=uk`)) || {};
                let dataEn = await advancedTmdbFetch(getTmdbEndpoint(`${path}?query=${encodeURIComponent(title)}&language=en`)) || {};
        ...
        if (bestResult) {
            ...
            if (!res.overview || !res.overview.trim()) {
                try {
                    ...
                    let enData = await advancedTmdbFetch(getTmdbEndpoint(`${enType}/${res.id}?language=en`));
    ...
    async function fetchCatalogPage(url, limit) {
        ...
        let tmdbItems = await processInQueue(links, processSingleItem); 
    ...
    async function fetchKinobazaCatalog(url, limit, noCache) {
        ...
        let tmdbItems = await processInQueue(items, async i => searchTmdbByTitleAndYear(i.title, i.year, i.type));
        let unique = {}, finalItems = tmdbItems.filter(i => i && i.id && (i.backdrop_path || i.poster_path) && !unique[i.id] && (unique[i.id] = true));
    ...
    // --- ГЛОБАЛЬНЕ ПЕРЕМАЛЮВАННЯ ТА ПЕРЕВІРКА ВСІХ КАРТОК НА ЕКРАНІ ---
    function forceRedrawAllCards() {
        try {
            var cardEls = document.querySelectorAll('.card, .card--wide-custom, .card--history-custom');
            cardEls.forEach(function(el) {
                if (el.lampaCardInstance && typeof el.lampaCardInstance.redrawBadgesAndAltDesign === 'function') {
                    el.lampaCardInstance.redrawBadgesAndAltDesign();
                }
            });
        } catch(e){}
    }

    function fetchLogo(movie, itemElement, forceLoad) {
        if (Lampa.Storage.get('uas_no_image_loading') === true && !forceLoad) return;

        var isHistoryCard = itemElement[0].classList.contains('card--history-custom');
        
        // Якщо працює ydes.js, не завантажуємо лого на звичайні картки
        if (window.YDesignLoaded && !isHistoryCard) return;

        var langPref = Lampa.Storage.get('ym_logo_lang') || 'uk_en';
        var quality = Lampa.Storage.get('ym_img_quality') || 'w300';
        
        var mType = movie.media_type || movie.method || movie.type;
        if (!mType) mType = (movie.name || movie.first_air_date) ? 'tv' : 'movie';
        ...
        var isHorz = isWideCustom || isHistoryCard;

        var alt2StandardPoster = Lampa.Storage.get('uas_alt2_standard_poster') === true;
        var needsCleanPoster = isAlt2 && !isHorz && !alt2StandardPoster;
        var needsLogo = langPref !== 'off' && (!isAlt1 || !isHorz) && (!isAlt2 || isHorz || !alt2StandardPoster);

        var viewEl = itemElement[0].querySelector('.card__view, .uas-history-view');
        if (!viewEl) return;

        var posClass = 'logo-pos-' + (Lampa.Storage.get(isHorz ? 'ym_logo_pos_hor' : 'ym_logo_pos_ver') || 'center');
        [].forEach.call(itemElement[0].querySelectorAll('.card-custom-logo, .card-custom-logo-text, .card-custom-logo-svg'), el => el.classList.remove('logo-pos-top', 'logo-pos-center', 'logo-pos-bottom'));
        ...
        function applyCleanPoster(url) {
            ...
            var cleanUrl = Lampa.TMDB.image('t/p/' + quality + url);
            var tmp = new Image();
            tmp.onload = function() {
                delete imgEl.src; 
                imgEl.src = cleanUrl;
        ...
        function renderLogo(logoUrl) {
            if (!needsLogo) return;
            [].forEach.call(itemElement[0].querySelectorAll('.card-custom-logo-text, .card-custom-logo-svg'), el => el.remove());
            if (logoUrl && logoUrl !== 'none') {
                ...
                if (!isHorz) sizeNum = Math.min(sizeNum + 30, 90);
        ...
        function renderTextLogo() {
            ...
            [].forEach.call(itemElement[0].querySelectorAll('.card-custom-logo-svg'), el => el.remove());
            
            var textLogoSize = parseFloat(Lampa.Storage.get('ym_text_logo_size') || '2');
            var useOriginal = langPref.indexOf('en') !== -1 && (movie.original_title || movie.original_name);
            var text = useOriginal ? (movie.original_title || movie.original_name) : (movie.title || movie.name);
            if (!text) return;
            
            var svg = document.createElementNS("http://www.w3.org/2000/svg", "svg");
            var charWidth = 14;
            var textWidth = Math.max(120, text.length * charWidth);
            svg.setAttribute("viewBox", "0 0 " + textWidth + " 40");
            svg.setAttribute("class", "card-custom-logo-svg " + posClass);
            svg.style.setProperty("max-height", textLogoSize + "em", "important");
            svg.style.setProperty("height", textLogoSize + "em", "important");
            svg.style.setProperty("width", "auto", "important");
            svg.style.setProperty("max-width", "90%", "important");
            ...
            textNode.style.fontFamily = "Roboto, Arial, sans-serif";
            textNode.style.filter = "drop-shadow(0px 2px 3px rgba(0,0,0,0.9))";
            textNode.textContent = text;
            
            svg.appendChild(textNode);
            viewEl.appendChild(svg);
        }

        if (!needsLogo && !needsCleanPoster) {
            [].forEach.call(itemElement[0].querySelectorAll('.card-custom-logo, .card-custom-logo-text, .card-custom-logo-svg'), el => el.remove());
            return;
        }
        ...
        if (!requireFetch) {
            ...
            if (needsLogo) {
                ...
                else [].forEach.call(itemElement[0].querySelectorAll('.card-custom-logo, .card-custom-logo-text, .card-custom-logo-svg'), el => el.remove());
            } else [].forEach.call(itemElement[0].querySelectorAll('.card-custom-logo, .card-custom-logo-text, .card-custom-logo-svg'), el => el.remove());
        } else {
            advancedTmdbFetch(getTmdbEndpoint(`${mType}/${movie.id}/images?include_image_language=uk,en,null`)).then(res => {
                ...
                if (needsLogo) {
                    ...
                    if (found) finalLogo = Lampa.TMDB.image('t/p/' + quality + found.file_path);
                ...
                if (needsLogo) {
                    if (needsCleanPoster && !isCleanFlag) [].forEach.call(itemElement[0].querySelectorAll('.card-custom-logo, .card-custom-logo-text, .card-custom-logo-svg'), el => el.remove());
        ...
        return {
            ...
            params: {
                ...
                emit: {
                    onCreate: function () {
                        var item = $(this.html);
                        item.removeClass('card').addClass('uas-title-btn-card uas-custom-card').empty(); 
                        if (!url) { item.removeClass('selector focusable'); item.addClass('uas-title-btn-static'); }
                        else { item.addClass('selector focusable'); }
        ...
        return {
            ...
            params: {
                ...
                emit: {
                    onCreate: function () {
                        var item = $(this.html);
                        item.removeClass('card').addClass('uas-collection-btn-card uas-custom-card selector focusable').empty(); 
    ...
    function makeFavoriteCardItem(bgUrl, fullBgUrl) {
        return {
            title: 'Закладки', is_title_btn: true,
            params: {
                createInstance: () => Lampa.Maker.make('Card', { title: 'Закладки' }, m => m.only('Card', 'Callback')),
                emit: {
                    onCreate: function () {
                        var item = $(this.html);
                        item.addClass('card--history-custom uas-protected-card');
                        
                        var htmlEl = item[0];
                        if (htmlEl) {
                            htmlEl._ydesign_built = true; // Захист від ydes.js
                        }
                        
                        var view = item.find('.card__view');
                        if (view.length) {
                            view.empty();
                            view.removeClass('card__view').addClass('uas-history-view');
                            
                            var bgImg = bgUrl ? `url(${bgUrl})` : 'none';
                            var bgColor = bgUrl ? 'transparent' : 'rgba(30,30,30,0.8)';
                            view.css({ 
                            ...
                            view.append('<div class="card-backdrop-overlay" style="background: rgba(0,0,0,0.65);"></div><div class="uas-history-content"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M19 21l-7-5-7 5V5a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2z"></path></svg><div>Закладки</div></div>');
                        }
                        
                        if (fullBgUrl && !window.uas_initial_bg_set) { window.uas_initial_bg_set = true; BgManager.change(fullBgUrl, true); }
                        item.on('hover:focus mouseenter', () => { if (fullBgUrl) BgManager.change(fullBgUrl); });
                    },
                    onlyEnter: function () { Lampa.Activity.push({ url: '', title: 'Закладки', type: 'book', component: 'favorite', source: 'tmdb', page: 1 }); }
                }
            }
        };
    }
                ...
                emit: {
                    onCreate: function () {
                        ...
                        if (view.length) {
                            ...
                            view.append('<div class="card-backdrop-overlay" style="background: rgba(0,0,0,0.65);"></div><div class="uas-history-content"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"></circle><polyline points="12 6 12 12 16 14"></polyline></svg><div>Історія</div></div>');
                        }
    ...
    function makeHistoryCardItem(movie) {
        // Клонуємо об'єкт фільму, щоб не забруднювати глобальний кеш (Lampa.Favorite), яким користуються інші плагіни
        var m = Object.assign({}, movie);
        m = normalizeMovieData(m, false); // false = не вантажити постер для клонованого об'єкта
        m.is_history_card = true; 
        
        // Передаємо прозорий піксель, щоб стандартний обробник карток Lampa не вантажив постер
        m.img = "data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7";

        return {
            title: m.title || m.name,
            params: {
                createInstance: () => Lampa.Maker.make('Card', m, mkr => mkr.only('Card', 'Callback')),
                emit: {
                    onCreate: function () {
                        var item = $(this.html);
                        item.addClass('card--history-custom uas-protected-card');
                        ...
                        if (view.length) {
                            ...
                            // Використовуємо оригінальний об'єкт movie для отримання правильних шляхів картинок
                            var imgUrlPath = movie.backdrop_path || movie.poster_path;
                            var quality = Lampa.Storage.get('ym_img_quality') || 'w300';
                            
                            var imgUrl = imgUrlPath ? Lampa.TMDB.image('t/p/' + quality + imgUrlPath) : '';
                            ...
                            if (getAltDesignType() === '0' && !isNaN(voteVal) && voteVal > 0 && Lampa.Storage.get('uas_badge_rating') !== false) {
                                view.append(`<div class="uas-history-vote">${voteVal.toFixed(1)}</div>`);
                            }
                        }
                        
                        if (!this.redrawBadgesAndAltDesign) {
                            this.redrawBadgesAndAltDesign = function() {
                                applyCardDesign(this, item[0], m);
                            };
                        }
                        this.redrawBadgesAndAltDesign();
                        
                        fetchLogo(m, item, true);
                        
                        var fullBgUrl = movie.backdrop_path ? Lampa.TMDB.image('t/p/w1280' + movie.backdrop_path) : '';
                        if (fullBgUrl && !window.uas_initial_bg_set) { window.uas_initial_bg_set = true; BgManager.change(fullBgUrl, true); }
                        item.on('hover:focus mouseenter', () => { var bg = movie.custom_full_bg || fullBgUrl; if (bg) BgManager.change(bg); });
                    },
                    // При кліку передаємо оригінальний movie об'єкт в Lampa.Activity для повноцінного завантаження інформації
                    onlyEnter: function () { Lampa.Activity.push({ url: '', component: 'full', id: movie.id, method: movie.media_type || movie.method || movie.type || (movie.name ? 'tv' : 'movie'), card: movie, source: movie.source || 'tmdb' }); }
                }
            }
        };
    }
            ...
            params: {
                ...
                emit: {
                    onCreate: function () {
                        ...
                        var imgUrl = (imgPath && Lampa.Storage.get('uas_no_image_loading') !== true) ? Lampa.TMDB.image('t/p/' + quality + imgPath) : '';
                        
                        view.css({ 'background-image': imgUrl ? `url(${imgUrl})` : 'none', 'background-size': 'cover', 'background-position': 'center', 'background-repeat': 'no-repeat', 'padding-bottom': '56.25%', 'height': '0', 'position': 'relative' });
                        view.append('<div class="card-backdrop-overlay"></div>');
                        ...
                        if (!this.redrawBadgesAndAltDesign) {
                            this.redrawBadgesAndAltDesign = function() {
                                applyCardDesign(this, item[0], movie);
                            };
                        }
                        this.redrawBadgesAndAltDesign();

                        if (Lampa.Storage.get('uas_no_image_loading') !== true) {
                        ...
                        var fullBgUrl = imgPath ? Lampa.TMDB.image('t/p/w1280' + imgPath) : '';
                        if (fullBgUrl && !window.uas_initial_bg_set && Lampa.Storage.get('uas_no_image_loading') !== true) { 
                            window.uas_initial_bg_set = true; 
                            BgManager.change(fullBgUrl, true); 
                        }
                        item.on('hover:focus mouseenter', () => { 
                            if (Lampa.Storage.get('uas_no_image_loading') === true) return;
                            var bg = movie.custom_full_bg || fullBgUrl; 
                            if (bg) BgManager.change(bg); 
                        });
                    },
            ...
            params: {
                ...
                emit: {
                    onCreate: function () {
                        ...
                        var nativeMode = Lampa.Storage.get('uas_native_cards_mode') === true;
                        if (!nativeMode && Lampa.Storage.get('uas_no_image_loading') !== true) {
                            fetchLogo(movie, item);
                        }
    ...
    function loadHistoryRow(callback) {
        ...
        let results =[], getRandImg = (arr) => {
            ...
            var quality = Lampa.Storage.get('ym_img_quality') || 'w300';
            return { img: Lampa.TMDB.image('t/p/' + quality + path), bg: Lampa.TMDB.image('t/p/w1280' + path) };
        };
    ...
    async function loadRow(urlId, loadUrl, title, callback) {
        ...
        try {
            ...
            let results = pageData && pageData.length ? pageData.map(getCardMapFn()) : [];
            callback({ results: results, title: '', source: 'uas_pro_source', uas_content_row: true, params: { items: { mapping: 'line', view: limit } } });
        } catch(e) { 
            callback({ results: [] }); 
        }
    }
    ...
    async function loadUaFlixRow(urlId, loadUrl, title, callback) {
        let limit = 9;
        let typeFilter = urlId === 'uaflix_films' ? 'films' : 'serials';
        try {
            let pageData = await fetchUaFlixCatalog(loadUrl, limit, false, typeFilter);
            let results = pageData && pageData.length ? pageData.map(getCardMapFn()) : [];
            callback({ results: results, title: '', source: 'uas_pro_source', uas_content_row: true, params: { items: { mapping: 'line', view: limit } } });
        } catch(e) { 
            callback({ results: [] }); 
    ...
    async function loadUaserialsCollectionsRow(urlId, loadUrl, title, callback) {
        try {
            ...
            let results = items && items.length ? items.slice(0, 7).map(makeCollectionButtonItem) : [];
            callback({ results: results, title: '', source: 'uas_pro_source', uas_content_row: true, params: { items: { mapping: 'line', view: 7 } } });
        } catch(e) { 
            callback({ results: [] }); 
        }
    }
    ...
    async function loadCommunityGemsRow(callback) {
        let limit = getCardsLimit();
        try {
            let res = await safeFetch(`https://wh.lme.isroot.in/v2/top?period=7d&top=asc&min_rating=7&per_page=${limit}&page=1`).then(r=>r.json()).catch(()=>({items:[]}));
            let mappedData = await getLmeTmdbItems(Array.isArray(res) ? res : (res.items ||[]));
            let results = mappedData && mappedData.length ? mappedData.map(getCardMapFn()) : [];
            callback({ results: results, title: '', source: 'uas_pro_source', uas_content_row: true, params: { items: { mapping: 'line', view: limit } } });
        } catch(e) { 
            callback({ results: [] }); 
        }
    ...
    async function loadRandomMoviesRow(callback) {
        try {
            ...
            let results = randData && randData.length ? randData.map(getCardMapFn()) : [];
            callback({ results: results, title: '', uas_content_row: true, params: { items: { mapping: 'line', view: 5 } } });
        } catch(e) { 
            callback({ results: [] }); 
        }
    }
    ...
    async function fetchPageData(targetPage, baseUrl, isLME, isKinobazaOnline, isUasCollection, isUasCollectionsList, isUaFlix, params, typeFilter) {
        if (isLME) {
            ...
            let normData = await fetchUaFlixCatalog(targetUrl, 20, false, typeFilter);
    ...
    Lampa.Api.sources.uas_pro_source = {
        list: async function (params, oncomplete, onerror) {
            ...
            let typeFilter = 'films';
            
            if (params.url === 'uaflix_films') { baseUrl = 'https://uafix.net/films/new_netflix_film_ua/'; isUaFlix = true; typeFilter = 'films'; }
            else if (params.url === 'uaflix_series') { baseUrl = 'https://uafix.net/serials/new_uaserial/'; isUaFlix = true; typeFilter = 'serials'; }
            ...
            try {
                ...
                if (page === 1) {
                    let r = await Promise.all([
                        fetchPageData(1, baseUrl, isLME, isKinobazaOnline, isUasCollection, isUasCollectionsList, isUaFlix, params, typeFilter),
                        fetchPageData(2, baseUrl, isLME, isKinobazaOnline, isUasCollection, isUasCollectionsList, isUaFlix, params, typeFilter)
                    ]);
                    res = { mapped: r[0].mapped.concat(r[1].mapped), total: r[0].total };
                } else {
                    res = await fetchPageData(page + 1, baseUrl, isLME, isKinobazaOnline, isUasCollection, isUasCollectionsList, isUaFlix, params, typeFilter);
                }
                
                if (page > 1) hideLoadingToast();
                if (res && res.mapped && res.mapped.length > 0) {
                    let normalizedResults = res.mapped.map(item => normalizeMovieData(item));
                    oncomplete({ results: normalizedResults, page: page, total_pages: res.total });
                } else onerror();
    ...
    function createSettings() {
        ...
        Lampa.SettingsApi.addParam({ component: 'ymainpage', param: { name: 'uas_lazy_load', type: 'trigger', default: true }, field: { name: 'Ліниве завантаження (Вимкнено плагіном)', description: 'Опція ігнорується, всі картки формуються миттєво задля швидкості та стабільності' } });
        Lampa.SettingsApi.addParam({ component: 'ymainpage', param: { name: 'uas_no_image_loading', type: 'trigger', default: false }, field: { name: 'Вимкнути завантаження зображень', description: 'Не завантажувати постери, логотипи та фони цим плагіном (дозволяє іншим плагінам дизайну самостійно керувати картинками)' } });
        ...
        Lampa.SettingsApi.addParam({ component: 'ymainpage', param: { name: 'ym_text_logo_size', type: 'select', values: { '1': '1em', '1.5': '1.5em', '2': '2em', '2.5': '2.5em', '3': '3em', '4': '4em', '5': '5em', '6': '6em', '7': '7em', '8': '8em', '9': '9em', '10': '10em' }, default: '2' }, field: { name: 'Розмір текста замість логотипу', description: 'Розмір шрифту текстового логотипу, якщо графічний відсутній (від 1em до 10em)' } });
        Lampa.SettingsApi.addParam({ component: 'ymainpage', param: { name: 'ym_row_title_size', type: 'select', values: { '1': '1em', '1.2': '1.2em', '1.4': '1.4em', '1.6': '1.6em', '1.8': '1.8em', '2': '2em', '2.5': '2.5em', '3': '3em', '4': '4em', '5': '5em' }, default: '1.4' }, field: { name: 'Розмір назв рядків', description: 'Шрифт для кнопок-розділювачів назв категорій на головній (від 1em до 5em)' } });
        ...
        Lampa.SettingsApi.addParam({ component: 'ymainpage', param: { name: 'uas_custom_proxies_btn', type: 'button' }, field: { name: 'Власні CORS проксі', description: 'Введіть через кому. Вони матимуть пріоритет' } });
        ...
        Lampa.SettingsApi.addParam({ component: 'ymainpage', param: { name: 'uas_text_desc_size', type: 'select', values: (function(){ var res={}; for(var i=7; i<=10.5; i+=0.5) { var v=(i/10).toFixed(2); res[v]=v; } return res; })(), default: '0.85' }, field: { name: 'Розмір тексту: Опис', description: '' } });
        ...
        Lampa.SettingsApi.addParam({ component: 'ymainpage', param: { name: 'uas_native_cards_mode', type: 'trigger', default: false }, field: { name: 'Стандартний стиль (Вимкнути все)', description: 'Вимкнути всі кастомні стилі горизонтальних/вертикальних карток і широкі форми' } });
        ...
        Lampa.SettingsApi.addParam({ component: 'ymainpage', param: { name: 'uas_badge_rating', type: 'trigger', default: true }, field: { name: 'Бейдж Рейтингу (TMDB)', description: 'Відображати standardну оцінку на картках' } });
        ...
        Lampa.Settings.listener.follow('open', function(e) {
            if (e.name === 'ymainpage') {
                ...
                e.body.find('[data-name="uas_custom_proxies_btn"]').on('hover:enter', function() { Lampa.Input.edit({ title: 'Власні CORS проксі', value: Lampa.Storage.get('uas_custom_proxies') || '', free: true, nosave: true }, function(v) { if (v !== undefined) { Lampa.Storage.set('uas_custom_proxies', v.trim()); Lampa.Noty.show('Збережено.'); } }); });
        ...
        Lampa.Settings.listener.follow('change', function(e) {
            var keys = ['uas_alt_design_type', 'uas_alt_badge_size', 'uas_text_hide', 'uas_text_align', 'uas_text_desc_align', 'uas_text_title_size', 'uas_text_desc_size', 'uas_bg_mode', 'uas_native_cards_mode', 'ym_row_title_size', 'uas_no_image_loading'];
    ...
    function overrideApi() {
        Lampa.Api.sources.tmdb.main = function (params, oncomplite, onerror) {
            var rowDefs =[
                ...
                { id: 'ym_row_uaflix_films', defOrder: 2, type: 'uaflix', url: 'uaflix_films', loadUrl: 'https://uafix.net/', title: 'Uaflix фільми', icon: 'https://yarikrazor-star.github.io/lmp/uaflix.png' },
                { id: 'ym_row_uaflix_series', defOrder: 3, type: 'uaflix', url: 'uaflix_series', loadUrl: 'https://uafix.net/', title: 'Uaflix серіали', icon: 'https://yarikrazor-star.github.io/lmp/uaflix.png' },
                { id: 'ym_row_movies_new', defOrder: 4, type: 'uas', url: 'uas_movies_new', loadUrl: 'https://uaserials.com/films/p/', title: 'Новинки фільмів', icon: 'https://upload.wikimedia.org/wikipedia/commons/5/51/Ukraine_film_clapperboard.svg' },
                { id: 'ym_row_series_new', defOrder: 5, type: 'uas', url: 'uas_series_new', loadUrl: 'https://uaserials.com/series/p/', title: 'Новинки серіалів', icon: 'https://upload.wikimedia.org/wikipedia/commons/3/3a/Mplayer.svg' },
                { id: 'ym_row_collections', defOrder: 6, type: 'uas_collections', url: 'uas_collections_list', loadUrl: 'https://uaserials.com/collections/', title: 'Підбірки', icon: 'https://upload.wikimedia.org/wikipedia/commons/e/ef/Film-award-stub.svg' },
                { id: 'ym_row_kinobaza', defOrder: 7, type: 'kinobaza', url: 'kinobaza_streaming', loadUrl: 'https://kinobaza.com.ua/online?order_by=date_desc&rating=1&rating_max=10&imdb_rating=1&imdb_rating_max=10&per_page=30&translated=has_ukr_audio&page=', title: 'Новинки Стрімінгів UA', icon: 'https://upload.wikimedia.org/wikipedia/commons/a/a5/Netflix_meaningful_logo.svg' },
                { id: 'ym_row_community', defOrder: 8, type: 'community', url: 'uas_community', title: 'Приховані геми LME', icon: 'https://upload.wikimedia.org/wikipedia/commons/b/b2/Anime_eye_film.png' },
                { id: 'ym_row_movies_watch', defOrder: 9, type: 'uas', url: 'uas_movies_pop', loadUrl: 'https://uaserials.my/filmss/w/', title: 'Популярні фільми', icon: 'https://upload.wikimedia.org/wikipedia/commons/7/7e/Filmreel-icon.svg' },
                { id: 'ym_row_series_pop', defOrder: 10, type: 'uas', url: 'uas_series_pop', loadUrl: 'https://uaserials.com/series/w/', title: 'Популярні серіали', icon: 'https://upload.wikimedia.org/wikipedia/commons/f/f6/Tvfilm.svg' },
                { id: 'ym_row_random', defOrder: 11, type: 'random', url: '', title: 'Випадкові фільми', icon: 'https://upload.wikimedia.org/wikipedia/commons/a/a3/Magicfilm_icon.svg' }
            ];
            ...
            Lampa.Api.partNext(parts_data, parts_data.length, oncomplite, onerror);
        };
    }
    ...
    function start() {
        ...
        if (!Lampa.Storage.get('ym_rows_init_v8_fix_16')) {
            ...
            if (Lampa.Storage.get('ym_text_logo_size') === null) Lampa.Storage.set('ym_text_logo_size', '2');
            if (Lampa.Storage.get('ym_row_title_size') === null) Lampa.Storage.set('ym_row_title_size', '1.4');
            if (Lampa.Storage.get('uas_bg_mode') === null) Lampa.Storage.set('uas_bg_mode', '1');
            if (Lampa.Storage.get('uas_native_cards_mode') === null) Lampa.Storage.set('uas_native_cards_mode', false);
            if (Lampa.Storage.get('uas_no_image_loading') === null) Lampa.Storage.set('uas_no_image_loading', false);
        }
        ...
        style.innerHTML = `
            .card.uas-hide-original-age .card__age { display: none !important; }
            ...
            .card--wide-custom .card-backdrop-overlay { position: absolute; top: 0; left: 0; right: 0; bottom: 0; background: rgba(0, 0, 0, 0.5); pointer-events: none; border-radius: inherit; z-index: 1; }
            ...
            .card-custom-logo { position: absolute; left: 50%; z-index: 5; filter: drop-shadow(0px 3px 5px rgba(0,0,0,0.8)); pointer-events: none; transition: opacity 0.3s ease, filter 0.3s ease; }
            ...
            .card-custom-logo-svg { position: absolute; left: 50%; z-index: 5; pointer-events: none; transition: filter 0.3s ease; }
            ...
            body:not(.uas-native-cards-mode) .card .card__title { text-align: var(--uas-text-align, center) !important; font-size: var(--uas-title-size, 1.0em) !important; }
            body.uas-hide-text:not(.uas-native-cards-mode) .card__title { display: none !important; }
            body.uas-hide-text .custom-title-bottom, body.uas-hide-text .custom-overview-bottom { display: none !important; }
            
            body:not(.uas-native-cards-mode) .card__vote, .card--wide-custom .card__vote { right: 0 !important; bottom: 0 !important; padding: 0.2em 0.45em !important; z-index: 2; position: absolute !important; font-weight: bold; background: rgba(0,0,0,0.6); }
            body:not(.uas-native-cards-mode) .card__type, .card--wide-custom .card__type { position: absolute !important; left: 0 !important; top: 0 !important; width: auto !important; height: auto !important; line-height: 1 !important; padding: 0.3em !important; background: rgba(0, 0, 0, 0.5) !important; display: flex !important; align-items: center; justify-content: center; z-index: 2; color: #fff !important; transition: background 0.3s !important; }
            ...
            body:not(.uas-alt-design-active):not(.uas-native-cards-mode) .card:not(.card--wide-custom) .card-badge-age { border-radius: 0 0.8em 0 0.8em !important; }
            body:not(.uas-alt-design-active):not(.uas-native-cards-mode) .card:not(.card--wide-custom) .card__vote { border-radius: 0.8em 0 0.8em 0 !important; }
            body:not(.uas-alt-design-active):not(.uas-native-cards-mode) .card:not(.card--wide-custom) .card__type { border-radius: 0.8em 0 0.8em 0 !important; }
            body:not(.uas-alt-design-active):not(.uas-native-cards-mode) .card:not(.card--wide-custom) .card__ua_flag { border-radius: 0 0.8em 0 0.8em !important; }
            ...
            .card .ext-rating-badge img { margin-right: 0.35em !important; width: 1.1em !important; height: 1.1em !important; object-fit: contain !important; filter: drop-shadow(0 1px 1px rgba(0,0,0,0.5)) !important; transition: opacity 0.3s ease; }
            ...
            .card--history-custom .card__title { display: none !important; }
            ...
            /* ========================================================================= */
            /* ПОВНІСТЮ ІЗОЛЬОВАНІ ЕЛЕМЕНТИ (ЗАХИСТ ВІД СТОРОННІХ ПЛАГІНІВ)              */
            /* ========================================================================= */
            ...
            .uas-title-btn-card.focus {
                background: rgba(255, 255, 255, 0.05) !important;
                border: 2px solid #fff !important;
                transform: scale(1.01) !important;
            }
            .uas-title-btn-card .title-btn-text {
                display: flex !important;
                align-items: center !important;
                font-size: var(--ym-row-title-size, 1.4em) !important;
                font-weight: bold !important;
                color: #777 !important;
                transition: color 0.2s ease !important;
            }
            .uas-title-btn-static {
                cursor: default !important;
            }
            .uas-title-btn-static .title-btn-text {
                opacity: 0.5 !important;
            }
            .uas-title-btn-card .title-btn-icon {
                height: 1.1em !important;
                width: auto !important;
                margin-right: 0.5em !important;
                filter: drop-shadow(0px 1px 2px rgba(0,0,0,0.5)) !important;
            }
            .uas-title-btn-card.focus .title-btn-text {
                color: #fff !important;
            }

            .uas-collection-btn-card {
                width: auto !important;
            ...
            .uas-collection-btn-card.focus {
                background: rgba(60,60,60,0.9) !important;
                border: 2px solid #fff !important;
            }
            .uas-collection-btn-card .collection-title {
            ...
            .card--history-custom.focus .uas-history-view {
                box-shadow: 0 10px 25px rgba(0,0,0,0.9) !important;
                border: 2px solid #fff !important;
                outline: none !important;
            }
            .card--history-custom > div:not(.uas-history-view) {
            ...
            @media (orientation: portrait), (max-width: 768px) {
                body:not(.uas-native-cards-mode) .items-line[data-uas-content-row="true"] .scroll__body > .card:not(.uas-custom-card):not(.card--history-custom),
                body:not(.uas-native-cards-mode) .items-line[data-uas-content-row="true"] .items-cards > .card:not(.uas-custom-card):not(.card--history-custom) {
                ...
                body:not(.uas-native-cards-mode) .card--wide-custom, .card--history-custom {
                    width: 44vw !important;
                    min-width: 44vw !important;
                    max-width: 44vw !important;
                }
                ...
                .uas-title-btn-card { margin: 0 !important; padding: 0.2em 1em !important; min-height: 2em !important; }
                .uas-title-btn-card .title-btn-text { font-size: calc(var(--ym-row-title-size, 1.4em) * 0.8) !important; }
            }
        `;
        ...
        Lampa.Listener.follow('activity', function(e) {
            if (e.type === 'start') {
                ...
                if (e.component === 'main' || e.component === 'tmdb' || !e.component) { 
                    document.body.classList.add('uas-main-active'); 
                    BgManager.show(); 
                    setTimeout(forceRedrawAllCards, 100);
                    setTimeout(forceRedrawAllCards, 1000);
                }
                else { document.body.classList.remove('uas-main-active'); BgManager.hide(); }
                updateDynamicStyles();
            }
        });
        Lampa.Listener.follow('controller', function(e) { if (e.type === 'focus' && !initialFocusHandled) { initialFocusHandled = true; if ($(e.target).hasClass('uas-title-btn-card')) setTimeout(function() { Lampa.Controller.move('down'); }, 20); } });

        // =========================================================================
        // ПЕРЕХОПЛЕННЯ ФАБРИКИ КАРТОК LAMPA ДЛЯ МИТТЄВОГО ОНОВЛЕННЯ ДИЗАЙНУ ТА БЛОКУВАННЯ ПОСТЕРІВ
        // =========================================================================
        var CardClassMap = Lampa.Maker ? Lampa.Maker.map('Card') : null;
        if (CardClassMap && CardClassMap.Card) {
            var origOnCreate = CardClassMap.Card.onCreate;
            CardClassMap.Card.onCreate = function () {
                var noImg = Lampa.Storage.get('uas_no_image_loading') === true;
                var isSpecial = this.data && (this.data.is_history_card || this.data.is_title_btn || this.data.is_collection_btn || this.data.is_placeholder);
                ...
                if (typeof origOnCreate === 'function') {
                    origOnCreate.apply(this, arguments);
                }

                if (this.html && this.data) {
                    var htmlEl = this.html[0] || this.html;
                    htmlEl.lampaCardInstance = this;
                    
                    if (!this.redrawBadgesAndAltDesign) {
                        this.redrawBadgesAndAltDesign = function() {
                            applyCardDesign(this, htmlEl, this.data);
                        };
                    }
                    this.redrawBadgesAndAltDesign();
                }
            ...
            var origOnVisible = CardClassMap.Card.onVisible;
            CardClassMap.Card.onVisible = function () {
                var noImg = Lampa.Storage.get('uas_no_image_loading') === true;
                var isSpecial = this.data && (this.data.is_history_card || this.data.is_title_btn || this.data.is_collection_btn || this.data.is_placeholder);

                if (noImg && !isSpecial) {
                    // Кажемо Lampa, що картинка "вже завантажена", щоб зупинити її внутрішній код
                    this.image_loaded = true;
                    // Встановлюємо прозорий 1х1 піксель як заглушку, щоб не було значка "розбитого зображення"
                    if (this.img && !this.img.src) {
                        this.img.src = "data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7";
                    }
                    // Імітуємо анімацію "з'явлення", бо Lampa зазвичай робить це після OnLoad
                    if (this.html) {
                        this.html.removeClass('card--preload').addClass('card--loaded');
                    }
                }

                if (typeof origOnVisible === 'function') {
                    origOnVisible.apply(this, arguments);
                } else {
                    this.visible = true; 
                }

                if (this.html && this.data && this.redrawBadgesAndAltDesign) {
                    this.redrawBadgesAndAltDesign();
                }
            };
        }
