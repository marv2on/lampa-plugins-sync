;(function() {
    ...
    function parseHTML(html) {
        try {
            ...
            // Жорстко вирізаємо всі скрипти (інлайн та зовнішні), стилі та iframe.
            // Це критично важливо для старих ТВ, оскільки jQuery намагається виконати 
            // сучасні JS-скрипти з чужих сайтів (наприклад, з UaKino), що викликає SyntaxError.
            safe = safe.replace(/<script[\s\S]*?<\/script>/gi, '');
            safe = safe.replace(/<style[\s\S]*?<\/style>/gi, '');
            safe = safe.replace(/<iframe[\s\S]*?<\/iframe>/gi, '');
            
            // Блокуємо завантаження картинок (щоб не було мережевих помилок в консолі)
            safe = safe.replace(/<img([^>]*)src\s*=/gi, '<img$1data-src=');
            safe = safe.replace(/<link([^>]*)href\s*=/gi, '<link$1data-href=');

            // Якщо доступний безпечний парсер jQuery - використовуємо його (3-й аргумент false забороняє виконання скриптів)
            if (typeof $.parseHTML === 'function') {
                try {
                    return $('<div>').append($.parseHTML(safe, document, false));
                } catch(err) {}
            }
    ...
    var UaCommentViewer = {
            ...
            if (window.Lampa && window.Lampa.Controller) {
                ...
                window.Lampa.Controller.add('uacom_viewer', {
                    ...
                    up: function() {
                        ...
                        if (body.length) {
                            body[0].scrollTop = body[0].scrollTop - 180;
                        }
                    },
                    down: function() {
                        var body = _this.element.find('.uacom-body');
        ...
        handleKeydown: function(e) {
            ...
            if (code === 37) { 
                ...
                if (bodyUp.length) bodyUp[0].scrollTop = bodyUp[0].scrollTop - 180;
            } else if (code === 40) { 
                e.preventDefault(); e.stopPropagation(); 
                var bodyDown = this.element.find('.uacom-body');
                if (bodyDown.length) bodyDown[0].scrollTop = bodyDown[0].scrollTop + 180;
