(function () {
    ...
    var CONFIG = { 
        ...
        timeout: 6000, // Мінімізовано таймаут
    ...
    var requestQueue = {
        activeCount: 0, queue:[], maxParallel: CONFIG.queue.maxParallel, // 10
    ...
    async function fetchUaFlixHtml(url, timeoutMs) {
        if (!timeoutMs) timeoutMs = 3000; // Мінімізовано
        for (let i = 0; i < PROXIES.length; i++) {
            try {
                let proxy = PROXIES[i];
                let fetchUrl = proxy.indexOf('?url=') !== -1 ? proxy + encodeURIComponent(url) : proxy + url;
    ...
    async function fetchUaFlixHtmlPost(url, paramsObj, timeoutMs) {
        if (!timeoutMs) timeoutMs = 4000; // Мінімізовано
    ...
    async function fetchUaFlixCatalog(url, limit, noCache) {
        ...
        let index = 0;
        let concurrency = 10; // Збільшено одночасні запити до 10
                    ...
                    (function(cIndex, cLink) {
                        Promise.resolve().then(async function() {
                            try {
                                ...
                                if (!details) {
                                    let itemHtml = await fetchUaFlixHtml(cLink, 2500); // Прискорений таймаут для списків
                                    if (itemHtml) {
    ...
    async function processInQueue(items, processFn, concurrency) {
        ...
        let workers = [];
        for (let w = 0; w < (concurrency || 10); w++) { // Збільшено за замовчуванням до 10
            workers.push((async () => {
    ...
    async function fetchCatalogPage(url, limit) {
        ...
        let tmdbItems = await processInQueue(links, processSingleItem, 10); // Збільшено
    ...
    async function fetchKinobazaCatalog(url, limit, noCache) {
        ...
        let tmdbItems = await processInQueue(items, async i => searchTmdbByTitleAndYear(i.title, i.year, i.type), 10); // Збільшено
        let unique = {}, finalItems = tmdbItems.filter(i => i && i.id && (i.backdrop_path || i.poster_path) && !unique[i.id] && (unique[i.id] = true));
    ...
    // Додано стандартну вертикальну картку для мобільних
    ...
    async function loadUaserialsCollectionsRow(urlId, loadUrl, title, callback) {
        try {
            ...
            // Підбірки завжди залишаються своїми
