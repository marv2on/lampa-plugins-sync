(function () {
    ...
    var BgManager = {
        ...
        change: function(url, instant) {
            ...
            if (instant) {
                ...
                // Затримка у 150мс, щоб не перевантажувати мережу при швидкому гортанні пультом
                this.timer = setTimeout(execute, 150);
            }
        },
        hide: function() {
            ...
            params: {
                ...
                emit: {
                    onCreate: function () {
                        ...
                        // Зміна фону при фокусі пультом або наведенні мишкою
                        var updateBg = function() {
                            if (fullBgUrl) BgManager.change(fullBgUrl);
                        };
                        item.on('hover:focus', updateBg);
    ...
    function start() {
        ...
        BgManager.init(); // Ініціалізація фонового менеджера
        ...
        style.innerHTML = `
            ...
            /* ---------- МОБІЛЬНИЙ / ВЕРТИКАЛЬНИЙ РЕЖИМ ---------- */
            @media (orientation: portrait), (max-width: 768px) {
                /* Усі картки стають розміру історії (1 рядок) */
                ...
                /* Ховаємо довгий опис, щоб картка була точно як у першому рядку */
                ...
                /* Мінімальні відступи між рядками */
        ...
        Lampa.Listener.follow('activity', function (e) {
            if (e.type === 'start') {
                ...
                window.uas_initial_bg_set = false; // Скидаємо прапорець для встановлення першого фону при завантаженні
                
                // Перевіряємо чи знаходимось ми на головній сторінці для активації фону
        ...
        CardMaker.Card.onVisible = function () {
            ...
            var isSpecialCard = isWideCard || isHistoryCard || isTitleBtn || isCollectionBtn || data.is_title_btn || data.is_collection_btn;
            ...
            if (isTitleBtn || isCollectionBtn || data.is_title_btn || data.is_collection_btn) return;
                ...
                if (meta) {
                    ...
                    if (cached === true) renderFlag(html);
                        ...
                        loadFlag(meta).then(function (isSuccess) {
                            if (isSuccess && cardInstance.html && cardInstance.html.parentNode) renderFlag(cardInstance.html);
                        });
                    }
                }
