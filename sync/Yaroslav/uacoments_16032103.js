(function() {
    ...
    /**
     * Безпечний парсер HTML, який не виконує скрипти.
     * Використовує DOMParser для надійного захисту від помилок виконання коду зі сторонніх сайтів.
     * @param {string} html - Рядок HTML для парсингу.
     * @returns {jQuery} - jQuery-об'єкт з розпарсеним та безпечним DOM-деревом.
     */
    function parseHTML(html) {
        try {
            // Повертаємо порожній, але валідний jQuery-об'єкт для некоректних вхідних даних
            if (typeof html !== 'string' || !html) return $();

            // Використовуємо нативний, безпечний DOMParser браузера. Він НЕ виконує скрипти.
            var parser = new DOMParser();
            var doc = parser.parseFromString(html, "text/html");

            // Повертаємо тіло документа, обгорнуте в jQuery, для подальшої роботи
            return $(doc.body);
        } catch (e) {
            console.error('UaComentsY: Критична помилка в parseHTML:', e);
            // У разі збою повертаємо порожній об'єкт, щоб уникнути подальших помилок
            return $();
        }
    }
    ...
    var UaCommentViewer = {
        ...
        show: function(commentsList, startIndex, prevCtrl, onClose) {
            if (this.active) this.close();
            this.active = true;
            this.comments = commentsList;
            this.currentIndex = startIndex;
            
            this.prevController = (prevCtrl && prevCtrl !== 'uacom_viewer') ? prevCtrl : 'full_start';
            ...
            this.element.find('.uacom-body').on('touchmove', function(e) {
                e.stopPropagation();
            });
        ...
        close: function() {
            ...
            var nextCtrl = this.prevController === 'uacom_viewer' ? 'full_start' : this.prevController;
            window.Lampa.Controller.toggle(nextCtrl);
            ...
            if (this.onCloseCallback) {
                this.onCloseCallback(this.currentIndex);
            }
        }
    };
    ...
    var network = {
        req: function(url, onSuccess, onError, proxyIdx) {
            ...
            $.ajax({
                ...
                method: 'GET',
                dataType: 'text', // Запитуємо дані як звичайний текст, щоб уникнути авто-виконання скриптів
                timeout: 8000,
                success: function(res) {
                    try {
                        // Іноді проксі повертає коротку помилку або пусту сторінку, ігноруємо її
    ...
    var parser = {
        parse: function(html, source, primarySelector) {
            ...
            try {
                ...
                if (!doc || !doc.length) return []; // Перевірка, чи парсер повернув валідний об'єкт
    ...
    UaCommentItem.prototype.create = function() {
        try {
            ...
            root.on('hover:enter click', function(e) {
                ...
                var prevController = 'full_start';
                if (window.Lampa && window.Lampa.Controller && window.Lampa.Controller.enabled()) {
                    prevController = window.Lampa.Controller.enabled().name;
                    if (prevController === 'uacom_viewer') prevController = 'full_start';
                }
                
                UaCommentViewer.show(_this.allComments, _this.index, prevController, function(newIndex) {
                    setTimeout(function() {
                        var targetCard = $('.ua-comment-card[data-idx="' + newIndex + '"]');
    ...
    function InlineComments() {
        ...
        this.init = function() {
            ...
            style.innerHTML = 
                ...
                ".uacom-body { padding: 35px 30px; overflow-y: auto; flex: 1; font-size: var(--uacom-noty-fsize, 1.3em); line-height: 1.6; color: #ddd; white-space: pre-wrap; }\n" +
