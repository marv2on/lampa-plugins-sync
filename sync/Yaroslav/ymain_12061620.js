(function () {
    ...
    // Оригінальний fetchHtml (без змін таймаутів)
    ...
    // Швидкий fetchHtml ДЛЯ UAFLIX з Promise.race для сумісності з Android TV
    ...
    // Швидкий fetchHtmlPost ДЛЯ UAFLIX з Promise.race
    async function fetchUaFlixHtmlPost(url, paramsObj, timeoutMs) {
        if (!timeoutMs) timeoutMs = 6000;
    ...
    async function fetchUaFlixCatalog(url, limit, noCache) {
        ...
        if (url.indexOf('page/') !== -1) {
            html = await fetchUaFlixHtml(url, 6000);
        } else {
            html = await fetchUaFlixHtmlPost(url, { 'xf_sort': 'get', 'xf_field': 'default', 'xf_value': 'date' }, 6000);
            if (!html || html.length < 100) html = await fetchUaFlixHtml(url, 6000); 
        }
        ...
        await new Promise(resolve => {
            ...
            function next() {
                ...
                while (active < concurrency && index < links.length && currentValidCount < limit) {
                    ...
                    // Використовуємо IIFE щоб замикання (closure) працювали коректно на старих версіях Android TV WebView
                    (function(cIndex, cLink) {
                        Promise.resolve().then(async function() {
                            try {
                                let details = uaFlixDetailsCache[cLink];
                                if (!details) {
                                    let itemHtml = await fetchUaFlixHtml(cLink, 3500);
                                    if (itemHtml) {
