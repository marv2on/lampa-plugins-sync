(function () {
	...
	function fetchImdbTrailer(imdb_id, source, quality, use_proxy, callback) {
        var qStr = String(quality);
		if (source === "imdbapi") {
			var api_url = "https://api.imdbapi.dev/titles/" + imdb_id + "/videos?types=trailer";
			if (use_proxy) api_url = "https://cors.bwa.workers.dev/" + api_url;
			...
			$.ajax({
				...
				success: function(data) {
					if (data && data.videos && data.videos.length > 0) {
						...
						var vid_id = video.id;
						var max_h = video.height || 1080;
						
                        var final_q = "1080p";
                        if (qStr === "1080") final_q = "1080p";
                        else if (qStr === "720") final_q = "720p";
                        else if (qStr === "480" || qStr === "sd") final_q = "480p";
                        else {
                            if (max_h >= 1080) final_q = "1080p";
                            else if (max_h >= 720) final_q = "720p";
                            else final_q = "480p";
                        }
                        
                        var req_val = parseInt(final_q);
                        if (req_val > max_h && max_h) {
                            if (max_h >= 1080) final_q = "1080p";
                            else if (max_h >= 720) final_q = "720p";
                            else final_q = "480p";
                        }

						var video_url = "https://imdb-video.media-imdb.com/mc/" + vid_id + "/" + vid_id + "_" + final_q + ".mp4";
						callback(use_proxy ? "https://cors.bwa.workers.dev/" + video_url : video_url);
					} else {
						callback(null);
					}
				},
			...
			$.ajax({
				...
				success: function(data) {
					if (data && data.trailer && data.trailer.playback) {
						...
						var startIndex = order.indexOf(qStr);
		...
		_createClass(LFUCache,[
			...
			},
			{
				...
				value: function skodf(e) {
                        ...
                        if (showImdbBtn) {
                            ...
                            if (render.find('.view--imdb_trailer').length === 0) {
                                ...
                                imdbBtn.on('hover:enter click', function() {
                                    ...
                                    if (btnSource !== "imdb" && btnSource !== "imdbapi") btnSource = "imdbapi";
                                    var btnQuality = Main.cases()[Main.stor()].field("cardify_button_imdb_quality") || "auto";

                                    Lampa.Noty.show('Завантаження IMDB трейлеру...');
                                    
                                    fetchImdbTrailer(imdb_id, btnSource, btnQuality, use_proxy, function(video_url) {
                                        if (video_url) {
                    ...
                    if (Main.cases()[Main.stor()].field("cardify_move_title")) {
						if (!(window.innerHeight > window.innerWidth || Lampa.Platform.is('mobile'))) {
                            var title = render.find('.full-start-new__title');
                            var mainContainer = render.find('.full-start-new__body');

                            if (title.length && mainContainer.length && render.find('.custom-title-wrapper').length === 0) {
                                mainContainer.css('position', 'relative');
                                
                                var titleTop = sanitizeEm(Main.cases()[Main.stor()].field("cardify_title_margin_top"), "0");
                                var titleLeft = sanitizeEm(Main.cases()[Main.stor()].field("cardify_title_margin_left"), "0");
                                var titleSize = sanitizeEm(Main.cases()[Main.stor()].field("cardify_title_size"), "3");

                                var wrapperTitle = $('<div class="custom-title-wrapper"></div>');
                                wrapperTitle.css({
                                    'position': 'absolute',
                                    'top': titleTop + 'em',
                                    'left': titleLeft + 'em',
                                ...
                                newTitle.removeClass('full-start-new__title');
                                newTitle.addClass('cardify-moved-title');
                                ...
                                wrapperTitle.append(newTitle);
                                mainContainer.append(wrapperTitle);
                            }
                        }
                    }
	...
	function startPlugin() {
		...
		Lampa.SettingsApi.addParam({
			component: "cardify",
			param: {
				name: "cardify_move_title",
				type: "trigger",
				default: false
			},
			field: {
				name: "Перенести назву",
				description: "Перемістити назву в лівий верхній кут (лише гориз. екрани)"
			}
		});
		Lampa.SettingsApi.addParam({
			component: "cardify",
			param: {
				name: "cardify_title_margin_top",
				type: "select",
                values: margins,
				default: "0"
			},
			field: {
				name: "Назва: Відступ зверху"
			}
		});
		Lampa.SettingsApi.addParam({
			component: "cardify",
			param: {
				name: "cardify_title_margin_left",
				type: "select",
                values: margins,
				default: "0"
			},
			field: {
				name: "Назва: Відступ зліва"
			}
		});
		Lampa.SettingsApi.addParam({
			component: "cardify",
			param: {
				...
				type: "select",
                values: {
                    "1.5": "1.5 em", "2.0": "2.0 em", "2.5": "2.5 em", "3": "3.0 em", "3.5": "3.5 em", "4.0": "4.0 em", "5.0": "5.0 em", "6.0": "6.0 em"
                },
				default: "3"
			},
			field: {
				name: "Назва: Розмір (текст/логотип)"
			}
		});
		Lampa.SettingsApi.addParam({
			component: "cardify",
		...
		Follow.get(Type.de([102, 117, 108, 108]), function (e) {
			if (Type.co(e)) {
                ...
                if (isRunTrailers || isBgTrailers) {
                    if (trailer_source === 'imdb' || trailer_source === 'imdbapi') {
                        var imdb_id = e.data.imdb_id || (e.data.external_ids ? e.data.external_ids.imdb_id : null) || (e.data.movie ? e.data.movie.imdb_id : null) || (e.data.tv ? e.data.tv.imdb_id : null) || (e.object && e.object.card ? e.object.card.imdb_id : null);
                        
                        if (imdb_id) {
                            fetchImdbTrailer(imdb_id, trailer_source, trailer_quality, use_proxy, function(video_url) {
                                if (video_url) {
                                    var tr = {
                                        type: 'imdb_video',
                                        url: video_url,
