(function () {
    ...
    // --- СИСТЕМА КЕШУВАННЯ ЗОБРАЖЕНЬ ALT DESIGN (300MB, 7 Днів) ---
    ...
    async function getImdbIdForTmdb(tmdbId, type) {
        ...
        let now = Date.now();
        if (cached && (now - cached.time) < 4 * 24 * 60 * 60 * 1000) {
            return cached.id;
        }
    ...
    function renderSeasonBadge(cardHtml, tmdbData, targetContainer) {
        ...
        if (currentSeason && last.season_number > 0) {
            ...
            var view = cardHtml.querySelector('.card__view');
            if (!view) return;
            var finalContainer = view.querySelector('.card-custom-badges') || view.querySelector('.card-left-badges') || targetContainer || view;
            ...
            if (Lampa.Storage.get('uas_alt_design_enable') !== true) {
    ...
    function fetchLogo(movie, itemElement) {
        ...
        // Вимкнення будь-яких логотипів та тексту
        if (langPref === 'off') {
        ...
        var useAltDesign = Lampa.Storage.get('uas_alt_design_enable') === true;
        var isWideCustom = itemElement.hasClass('card--wide-custom');
        
        // Для звичайних широких карток в Альт-режимі прибираємо лого (ТІЛЬКИ для Новинок/Широких)
        // Картки Історії (card--history-custom) зберігають свої логотипи!
        if (useAltDesign && isWideCustom) {
    ...
    function createSettings() {
        ...
        Lampa.SettingsApi.addComponent({
            component: 'ymainpage_alt',
            name: 'Альтернативний вигляд',
        ...
        Lampa.SettingsApi.addParam({
            component: 'ymainpage_alt',
            param: { name: 'uas_alt_design_enable', type: 'trigger', default: false },
            field: { name: 'Увімкнути альтернативний вигляд', description: 'Новий дизайн карток та зображення з easyratingsdb' }
        });

        Lampa.SettingsApi.addParam({
            component: 'ymainpage_alt',
            param: { name: 'uas_alt_apikey_btn', type: 'button' },
            field: { name: 'easyratingsdb Api key', description: 'Натисніть, щоб ввести ваш ключ' }
        });
    ...
    function start() {
        ...
        style.innerHTML = `
            ...
            body.uas-alt-design-active .card--wide-custom .card-backdrop-overlay { display: none !important; background: transparent !important; }
            body.uas-alt-design-active .card--wide-custom .card__view::after,
            body.uas-alt-design-active .card--wide-custom .card__view::before { display: none !important; content: none !important; }
            
            /* Динамічний контейнер бейджів з 6 позиціями */
            .card.uas-alt-card .card-custom-badges {
            ...
            .card.uas-alt-card .badge-pos-tl { left: 0.3em !important; top: 0.55em !important; align-items: flex-start !important; }
            .card.uas-alt-card .badge-pos-tr { right: 0.3em !important; top: 0.55em !important; align-items: flex-end !important; }
            .card.uas-alt-card .badge-pos-ml { left: 0.3em !important; top: 50% !important; transform: translateY(-50%) !important; align-items: flex-start !important; }
            .card.uas-alt-card .badge-pos-mr { right: 0.3em !important; top: 50% !important; transform: translateY(-50%) !important; align-items: flex-end !important; }
            .card.uas-alt-card .badge-pos-bl { left: 0.3em !important; bottom: 0.55em !important; align-items: flex-start !important; }
            .card.uas-alt-card .badge-pos-br { right: 0.3em !important; bottom: 0.55em !important; align-items: flex-end !important; }
            
            .card.uas-alt-card .card-custom-badges > div {
            ...
            /* Порядок: Зверху (стандартно) */
            .card.uas-alt-card .card-custom-badges .card-badge-age { order: 1 !important; }
            .card.uas-alt-card .card-custom-badges .card__type { order: 2 !important; }
            .card.uas-alt-card .card-custom-badges .card__ua_flag { order: 3 !important; }

            /* Порядок: Знизу (Рік знизу, Сезон, UA зверху) */
            .card.uas-alt-card .badge-pos-bl .card__ua_flag, .card.uas-alt-card .badge-pos-br .card__ua_flag { order: 1 !important; }
            .card.uas-alt-card .badge-pos-bl .card__type,    .card.uas-alt-card .badge-pos-br .card__type    { order: 2 !important; }
            .card.uas-alt-card .badge-pos-bl .card-badge-age,.card.uas-alt-card .badge-pos-br .card-badge-age{ order: 3 !important; }

            /* Порядок: Посередині (Рік всередині) */
            .card.uas-alt-card .badge-pos-ml .card__ua_flag, .card.uas-alt-card .badge-pos-mr .card__ua_flag { order: 1 !important; }
            .card.uas-alt-card .badge-pos-ml .card-badge-age,.card.uas-alt-card .badge-pos-mr .card-badge-age{ order: 2 !important; }
            .card.uas-alt-card .badge-pos-ml .card__type,    .card.uas-alt-card .badge-pos-mr .card__type    { order: 3 !important; }

            .card.uas-alt-card .card-custom-badges .card__ua_flag { 
                background: rgba(0, 0, 0, 0.55) !important; 
                border: 1px solid rgba(255,255,255,0.15) !important;
                display: flex !important;
        ...
        CardMaker.Card.onVisible = function () {
            ...
            if (useAltDesign && view) {
                html.classList.add('uas-alt-card');
                targetContainer = view.querySelector('.card-custom-badges');
                if (!targetContainer) {
                    targetContainer = document.createElement('div');
                    targetContainer.className = 'card-custom-badges badge-pos-' + badgePos;
                    view.appendChild(targetContainer);
                }
            } else if (html.classList) {
                html.classList.remove('uas-alt-card');
                var oldBadges = view.querySelector('.card-custom-badges');
                if(oldBadges) targetContainer = oldBadges;
            }
            ...
            // --- ЗАВАНТАЖЕННЯ ЗОБРАЖЕНЬ: TMDB -> ALT DESIGN KESH ---
            if (useAltDesign && data.id && altToken) {
                if (!isHistoryCard) { // Для історії не завантажуємо easyratingsdb
