            ...
            $.ajax({
                ...
                success: function(res) {
                    try {
                        ...
                        // ЗМЕНШЕНО ЛІМІТ до 50 символів! Це дозволить не втрачати короткі HTML-редиректи Кінобази.
                ...
                if (useKinoBaza) {
                    try {
                        ...
                        if (!imdbId) {
                            ...
                            // Крок 1: Запит ЧЕРЕЗ CORS ПРОКСІ на /search?q=
                            network.req(kbSearchUrl, function(html) {
                                if (kbCompleted) return;
                                
                                // Якщо проксі автоматично пройшов редирект і видав сторінку фільму
                                ...
                                // Якщо проксі повернув сторінку-заглушку (з редиректом), шукаємо лінк на сам фільм
                                ...
                                var checkNextLink = function() {
                                    ...
                                    // Крок 2: Знайшли лінк на фільм, огортаємо його знову в CORS ПРОКСІ
                                    network.req(links[lIdx++], function(pageHtml) {
                                        if (kbCompleted) return;
